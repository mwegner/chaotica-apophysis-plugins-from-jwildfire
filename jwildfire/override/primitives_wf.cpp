/*
    Apophysis Plugin: primitives_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/PrimitivesWFFunc.java
...
const int SHAPE_SQUARE = 3;
const int SHAPE_TRIANGLE = 4;
const int SHAPE_PYRAMID = 5;
const int SHAPE_TORUS = 6;
const double P_COSA = 0.5;
const double P_SINA = 0.86602540378443864676372317075293618347140262690519031402790348972596650845440001854057309337862428783781307070770; //sqrt(3.0) / 2.0;
...
#define APO_VARIABLE_PREFIX "primitives_wf_"
#include "plugin.h"

APO_PLUGIN("primitives_wf");
APO_VARIABLES(
        VAR_INTEGER(shape, SHAPE_SPHERE),
        VAR_REAL(a, 1.0),
        VAR_REAL(b, 1.0),
        VAR_REAL(c, 1.0),
...
void createSquare (Variation* vp) {

    double size = VVAR;
    double dx = 0.0, dy = 0.0;
    bool left, top;

    if (VAR(filled) == 1 && GOODRAND_01() < 0.5) {
        dx = size * (GOODRAND_01() - 0.5);
        dy = size * (GOODRAND_01() - 0.5);
    } else {
        switch (random0x(2)) {
            case 0:
                left = GOODRAND_01() < 0.5;
                if (VAR(filled) == 0 || GOODRAND_01() < 0.05) {
...
bool insideUnitTriangle (Variation* vp, double pX, double pY) {
...
void createTriangle (Variation* vp) {

    double dx = 0.0, dy = 0.0;
    while (true) {
        dx = GOODRAND_01();
        dy = GOODRAND_01();
        if (insideUnitTriangle(vp, dx, dy)) {
            break;
        }
    }
    FPx += GOODRAND_01() < 0.5 ? dx : -dx;
    FPy += -dy;
    if (true /* pContext\.isPreserveZCoordinate() */) {
        FPz += VVAR * FTz;
    }
...
void createPyramid (Variation* vp) {
    ...
    switch (side) {
        ...
        default:
            ...
            while (true) {
                tx = GOODRAND_01();
                ty = GOODRAND_01();
                if (insideUnitTriangle(vp, tx, ty)) {
                    break;
                }
            }
            tx *= size;
...
void createSphere (Variation* vp) {

    double radius = VAR(filled) == 0 ? VVAR : GOODRAND_01() < 0.05 ? VVAR : VVAR * GOODRAND_01();

    double VAR(a) = GOODRAND_01() * 2 * M_PI;
    double sina = sin(VAR(a));
    double cosa = cos(VAR(a));

    double VAR(b) = GOODRAND_01() * 2 * M_PI;
    double sinb = sin(VAR(b));
    double cosb = cos(VAR(b));

    FPx += radius * sina * cosb;
    FPy += radius * sina * sinb;
    FPz += radius * cosa;
}

void createDisc (Variation* vp) {
...
void createTorus (Variation* vp) {

    double radius = VAR(filled) == 0 ? VAR(a) : GOODRAND_01() < 0.05 ? VAR(a) : VAR(a) * GOODRAND_01();
    double alpha = GOODRAND_01() * 2 * M_PI;

    double x = radius * cos(alpha);
    double y = radius * sin(alpha);

    double beta = GOODRAND_01() * 2 * M_PI;

    FPy += x * VVAR;
    FPx += (y + VAR(b) + VAR(b)) * sin(beta) * VVAR;
    FPz += (y + VAR(b) + VAR(b)) * cos(beta) * VVAR;
}
