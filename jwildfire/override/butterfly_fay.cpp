/*
    Apophysis Plugin: butterfly_fay

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/ButterflyFayFunc.java
...
// matthew:  moved to overwrite due to bad linting on these constants


#include "datahelpers.h"

const double cycle_length = 2 * M_PI; // 2(PI)
const double radians_to_close = 2 * M_PI * M_PI * M_PI; // 2(PI)^3
const double cycles_to_close = radians_to_close / cycle_length; // = PI^2

typedef struct
{
	double cycles; // number of cycles (2*PI radians, circle circumference), if set to 0 then number of cycles is calculated automatically
	double offset; // offset c from equations
	int unified_inner_outer;
	int outer_mode;
	int inner_mode;
	double outer_spread; // deform based on original x/y
	double inner_spread; // deform based on original x/y
	double outer_spread_ratio; // how much outer_spread applies to x relative to y
	double inner_spread_ratio; // how much outer_spread applies to x relative to y
	double spread_split;
	double fill;
	double _number_of_cycles; // 1 cycle = 2*PI

} Variables;

#define APO_VARIABLE_PREFIX "butterfly_fay_"
#include "plugin.h"

APO_PLUGIN("butterfly_fay");
APO_VARIABLES(
	VAR_REAL(cycles, 0),
	VAR_REAL(offset, 0),
	VAR_INTEGER(unified_inner_outer, 1),
...
{
    // setting cyclesParam to 0 is meant to indicate the function should determine how many VAR(cycles) 
    //     (actually setting VAR(cycles) = 0 will just yield a single point)
    //  for the butterfly curve I am taking that to mean closing the curve, which as noted above 
    //      I have observationally determined is at exactly 2(PI)^3 radians, which is same as (PI)^2 VAR(cycles)
    if (VAR(cycles) == 0) {
      VAR(_number_of_cycles) = cycles_to_close;
    } else {
      VAR(_number_of_cycles) = VAR(cycles);
    }
...
{
    double theta = atan2(FTy, FTx); // atan2 range is [-PI, PI], so covers 2PI, or 1 cycle
    double t = VAR(_number_of_cycles) * theta;
    // r = e^cos(t) - 2cos(4t) - sin^5(t/12)
    // y = sin(t)*r
    // x = cos(t)*r
    double rin = VAR(spread_split) * sqrt((FTx * FTx) + (FTy * FTy));
    double r = 0.5 * (exp(cos(t)) - (2 * cos(4 * t)) - pow(sin(t / 12), 5) + VAR(offset));
    // a "fully" parameterized version:
    // double r = 0.5 * ( (m1 * exp(m4 * cos(t + b1) + b4)) - (m2 * 2 * (cos(m5*4*t + b2) + b5)) - (m3 * pow(m6 * (sin(m7 * t/12 + b3) + b6), (m8 * 5 + b7))) + VAR(offset));

    if (VAR(fill) != 0) {
      r = r + (VAR(fill) * (random01() - 0.5));
    }
    ...
    if ((abs(rin) > abs(r)) || (VAR(unified_inner_outer) == 1)) { // incoming point lies "outside" of curve OR ignoring inner_outer distinction
      switch (VAR(outer_mode)) {
        ...
        case 2:
          ...
          FPx += VVAR * (x + (VAR(outer_spread) * VAR(outer_spread_ratio) * (xin - x)));
          FPy += VVAR * (y + (VAR(outer_spread) * (yin - y)));
          break;
        case 3:
          xin = abs(FTx);
          yin = abs(FTy);
          ...
          FPx += VVAR * (x + (VAR(outer_spread) * VAR(outer_spread_ratio) * xin));
          FPy += VVAR * (y + (VAR(outer_spread) * yin));
          break;
        case 4:
          ...
          FPy += VVAR * riny * y;
          break;
        case 5: // original butterfly2 implementation (same as VAR(outer_mode) 3, but without the sign modifications)
          FPx += VVAR * (x + (VAR(outer_spread) * VAR(outer_spread_ratio) * FTx));
          FPy += VVAR * (y + (VAR(outer_spread) * FTy));
          break;
        default:
          FPx += VVAR * x;
          FPy += VVAR * y;
      ...
      switch (VAR(inner_mode)) {
        ...
        case 2:
          ...
          FPx += VVAR * (x - (VAR(inner_spread) * VAR(inner_spread_ratio) * (x - xin)));
          FPy += VVAR * (y - (VAR(inner_spread) * (y - yin)));
          break;
        case 3:
          xin = abs(FTx);
          yin = abs(FTy);
          ...
          FPx += VVAR * (x - (VAR(inner_spread) * VAR(inner_spread_ratio) * xin));
          FPy += VVAR * (y - (VAR(inner_spread) * yin));
          break;
        case 4:
          ...
          FPy += VVAR * riny * y;
          break;
        case 5: // original butterfly2 implementation (same as VAR(inner_mode) 3, but without the sign modifications)
          FPx += VVAR * (x + (VAR(inner_spread) * VAR(inner_spread_ratio) * FTx));
          FPy += VVAR * (y + (VAR(inner_spread) * FTy));
          break;
        default:
          FPx += VVAR * x;
          FPy += VVAR * y;
