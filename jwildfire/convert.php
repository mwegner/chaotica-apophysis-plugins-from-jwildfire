...
DONE;
    ...
    // use our template
    $template = file_get_contents("template.cpp");
    $search = array();
    $replace = array();
    ...
    file_put_contents('out/' . $filename . '.cpp', $template);
}

// bring in our replacement files (system call, so would break on windows)
`cp overwrite/* out/`;
