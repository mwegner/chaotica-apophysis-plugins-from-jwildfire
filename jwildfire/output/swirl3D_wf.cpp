...
int PluginVarCalc(Variation* vp)
{
    ...
    double ang = atan2(FTx, FTy);   // + log(rad)*shift;


    //   double ang = GOODRAND_01()*360.0;
    //   double rad=  -1.0 + GOODRAND_01()*2.0;

    FPx += VVAR * (rad * cos(ang));
    FPy += VVAR * (rad * sin(ang));
    FPz += VVAR * (sin(6.0 * cos(rad) - VAR(n) * ang));
    TC = abs(sin(6.0 * cos(rad) - VAR(n) * ang));
