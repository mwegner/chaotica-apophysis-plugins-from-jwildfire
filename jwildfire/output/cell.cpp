...
int PluginVarCalc(Variation* vp)
{
    ...
    double inv_cell_size = 1.0 / VAR(size);

    /* calculate input cell */
    int x = (int) floor(FTx * inv_cell_size);
    int y = (int) floor(FTy * inv_cell_size);

    /* Offset from cell origin */
    double dx = FTx - x * VAR(size);
    double dy = FTy - y * VAR(size);
    ...
    FPx += VVAR * (dx + x * VAR(size));
    FPy -= VVAR * (dy + y * VAR(size));
