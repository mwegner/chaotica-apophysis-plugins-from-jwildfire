...
int PluginVarCalc(Variation* vp)
{
    ...
    double r = sqrt(1.25) * sqrt((double) VAR(_m));
    int c = (int) ((double) VAR(_m) * GOODRAND_01());
    double ra = 1.0 / (sqrt(3.0) * sqrt(FTx * FTx + FTy * FTy));

    if (c % 2 == 0) {
      x = -FTx / r + ra * FTy / r + VAR(_a)[c];
      y = -ra * FTx / r - FTy / r + VAR(_b)[c];
    } else {
      x = FTx / r + ra * FTy / r + VAR(_a)[c];
      y = -ra * FTx / r + FTy / r + VAR(_b)[c];
    }
