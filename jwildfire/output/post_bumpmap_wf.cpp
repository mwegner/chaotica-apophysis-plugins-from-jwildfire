/*
    Apophysis Plugin: post_bumpmap_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/PostBumpMapWFFunc.java
...
APO_PLUGIN("post_bumpmap_wf");
APO_VARIABLES(
	VAR_REAL(scale_x, 1.0),
	VAR_REAL(scale_y, 1.0),
	VAR_REAL(scale_z, 0.2),
...
void setRessource (Variation* vp, String pName, byte[] pValue) {
    ...
    if (RESSOURCE_IMAGE_FILENAME.equalsIgnoreCase(pName)) {
      ...
      if (imageFilename != null) {
        inlinedImage = null;
        VAR(_inlinedImageHash) = 0;
      }
      bumpMap = null;
    } else if (RESSOURCE_INLINED_IMAGE.equalsIgnoreCase(pName)) {
      inlinedImage = pValue;
      VAR(_inlinedImageHash) = RessourceManager.calcHashCode(inlinedImage);
...
int PluginVarPrepare(Variation* vp)
{
    ...
    if (inlinedImage != null) {
      try {
        bumpMap = RessourceManager.getImage(VAR(_inlinedImageHash), inlinedImage);
      } catch (Exception e) {
        e.printStackTrace();
      }
    ...
    VAR(_imgWidth) = bumpMap.getImageWidth();
    VAR(_imgHeight) = bumpMap.getImageHeight();
...
{
    double x = (FTx - VAR(offset_x) + 1.0) / VAR(scale_x) * 0.5 * (double) (VAR(_imgWidth) - 1);
    double y = (FTy - VAR(offset_y) + 1.0) / VAR(scale_y) * 0.5 * (double) (VAR(_imgHeight) - 1);
