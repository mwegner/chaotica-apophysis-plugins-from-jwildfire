/*
    Apophysis Plugin: inverted_julia

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Inverted_JuliaFunc.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    //Based on the Julia variation but with an adjustable inward VAR(center) and variables.

    double x = FTx;
    double y = FTy;
    double xs = x * x;
    double ys = y * y;
    double z = pow(xs + (ys * VAR(_y2_mult)), VAR(power)) + VAR(_x2y2_add);

    double q = atan2(x * VAR(_a2x_mult), y * VAR(_a2y_mult) + VAR(_a2y_add)) * 0.5 + M_PI * (int) (2 * GOODRAND_01());

    FPx += VVAR * cos(z * VAR(_cos_mult)) * (sin(q) / z / VAR(center));
    FPy += VVAR * cos(z * VAR(_cos_mult)) * (cos(q) / z / VAR(center)) * VAR(_y_mult);
