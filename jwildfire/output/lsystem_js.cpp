/*
    Apophysis Plugin: lsystem_js

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/LsystemFunc.java
...
typedef struct
{
	...
	double angle;
	int presetId;
	int show_lines_param;
	double line_thickness;
	double _line_thickness;
	int show_points_param;
	double point_thickness_param;
...
APO_PLUGIN("lsystem_js");
APO_VARIABLES(
	VAR_INTEGER(iters, 3),
	VAR_REAL(step, 0.1),
	VAR_REAL(angle, 0.0),
	VAR_INTEGER(presetId, -1),
	VAR_INTEGER(show_lines_param, 1),
...
{
    GOODRAND_PREPARE();
    double show_sum = VAR(show_lines_param) + VAR(show_points_param);

    VAR(_line_fraction) = VAR(show_lines_param) / show_sum;
    VAR(_point_fraction) = VAR(show_points_param) / show_sum;
    ...
    VAR(_point_thickness) = VAR(point_thickness_param) / 100;
    ...
    */
    ...
    try {
      ...
      expanded = ls.processFile(string, VAR(iters));
    } catch (Exception e) {
      e.printStackTrace();
    } finally {
    ...
    if (VAR(angle) == 0.0)
      ...
      canvas.setAngle(VAR(angle));
    //     System.out.println("File Angle: " + file_angle);
    canvas.setStep(VAR(step));
    try {
