/*
    Apophysis Plugin: vibration2

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Vibration2Func.java
...
APO_PLUGIN("vibration2");
APO_VARIABLES(
	VAR_REAL(dir, 0.0),
	VAR_REAL(angle, 1.5708),
	VAR_REAL(freq, 1.0),
	VAR_REAL(amp, 0.25),
	VAR_REAL(phase, 0.0),
	VAR_REAL(dir2, 1.5708),
	VAR_REAL(angle2, 1.5708),
	VAR_REAL(freq2, 1.0),
	VAR_REAL(amp2, 0.25),
	VAR_REAL(phase2, 0.0),
	VAR_REAL(dm, 0.0),
	...
	VAR_REAL(amfreq, 0.1),
	VAR_REAL(d2m, 0.0),
	VAR_REAL(d2mfreq, 0.1),
	VAR_REAL(t2m, 0.0),
	VAR_REAL(t2mfreq, 0.1),
	VAR_REAL(f2m, 0.0),
...
double modulate (Variation* vp, double amp, double freq, double x) {

    return VAR(amp) * cos(x * VAR(freq) * M_2PI);
}
