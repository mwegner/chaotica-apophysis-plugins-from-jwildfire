/*
    Apophysis Plugin: mobius_strip

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/MobiusStripFunc.java
...
typedef struct
{
	...
	double _rotate_x;
	double _rotate_y;
	double _modify_z = 1;  // default to full 3D
	int _width_mode = WRAP;  // default to wrapping all x coords around the strip
	int _radial_mode = WRAP; // default to wrapping all y coords across the strip
	double _xmin;
	double _xmax;
	double _ymin;
	double _ymax;
...
int PluginVarPrepare(Variation* vp)
{
   ...
   VAR(_range_y) = 1;
   VAR(_modify_z) = 1;
   VAR(_width_mode) = WRAP;
   VAR(_radial_mode) = WRAP;
