/*
    Apophysis Plugin: pTransform

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/PTransformFunc.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    rho = (VAR(use_log) != 0) ? log((sqrt(FTx * FTx + FTy * FTy) + SMALL_EPSILON)) / VAR(power) + VAR(move) : (sqrt(FTx * FTx + FTy * FTy) + SMALL_EPSILON) / VAR(power) + VAR(move);
    theta = atan2(FTx, FTy) + VAR(rotate);
    ...
    if (VAR(use_log) != 0) rho = exp(rho);

    FPx += VVAR * rho * cos(theta);
    FPy += VVAR * rho * sin(theta);
