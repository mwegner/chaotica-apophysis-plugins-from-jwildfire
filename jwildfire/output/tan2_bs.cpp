/*
    Apophysis Plugin: tan2_bs

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Tan2_BSFunc.java
...
#define APO_VARIABLE_PREFIX "tan2_bs_"
#include "plugin.h"
...
int PluginVarCalc(Variation* vp)
{
    ...
    //Tangent TAN
    double tansin = sin(VAR(x1) * FTx);
    double tancos = cos(VAR(x2) * FTx);
    double tansinh = sinh(VAR(y1) * FTy);
    double tancosh = cosh(VAR(y2) * FTy);
    double d = (tancos + tancosh);
    if (d == 0) {
      return TRUE;
    }
    double tanden = 1.0 / d;
    FPx += VVAR * tanden * tansin;
    FPy += VVAR * tanden * tansinh;
