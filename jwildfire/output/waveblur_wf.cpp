/*
    Apophysis Plugin: waveblur_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/WaveBlurWFFunc.java
...
APO_PLUGIN("waveblur_wf");
APO_VARIABLES(
	VAR_INTEGER(count, 5),
	VAR_REAL(amplitude_z, 0.5),
	VAR_REAL(phase, 0.0),
...
int PluginVarCalc(Variation* vp)
{
    ...
    double rnd = 1.0 - 2.0 * GOODRAND_01();
    double r;
    if (GOODRAND_01() < 0.5) {
      r = acos(rnd) - (GOODRAND_0X(VAR(count)) + 1) * M_PI + VAR(phase);
    } else {
      r = acos(-rnd) - (GOODRAND_0X(VAR(count)) + 1) * M_PI + VAR(phase);
    }
    r *= VVAR / (VAR(count) * M_PI);
    ...
    if (VAR(direct_color) == 1) {
      TC = limitVal(acos(rnd) * VAR(color_scale) + VAR(color_offset), 0.0, 1.0);
    }
    if (VAR(amplitude_z) != 0.0) {
      double zamp = (cos(rnd) - 0.5) * VAR(amplitude_z);
      if (fabs(VAR(damping_z)) > SMALL_EPSILON) {
        double dmp = r * VAR(damping_z);
        zamp *= exp(dmp);
      }
      FPz += VVAR * zamp;
    }
