/*
    Apophysis Plugin: hypershift2

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Hypershift2Func.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    double pq = M_PI / VAR(q);
    double pp = M_PI / VAR(p);
    double spq = sin(pq);
    double spp = sin(pp);
    ...
    double FX = FTx * scale2;
    double FY = FTy * scale2;

    double rad = 1 / (FX * FX + FY * FY);
    double x = rad * FX + shift;
    double y = rad * FY;
    rad = VVAR * scale / (x * x + y * y);
    double angle = ((GOODRAND_0X(INT_MAX) % VAR(p)) * 2.0 + 1.0) * M_PI / VAR(p);
