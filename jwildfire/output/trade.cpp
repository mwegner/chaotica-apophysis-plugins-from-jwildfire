...
{
    GOODRAND_PREPARE();
    VAR(_c1) = VAR(r1) + VAR(d1);
    VAR(_c2) = VAR(r2) + VAR(d2);
...
int PluginVarCalc(Variation* vp)
{
    ...
    if (FTx > 0.0) {
      double r = sqrt(sqr(VAR(_c1) - FTx) + sqr(FTy));
      if (r <= VAR(r1)) {
        r *= VAR(r2) / VAR(r1);
        double a = atan2(FTy, VAR(_c1) - FTx);
        double s = sin(a);
        double c = cos(a);
        FPx += VVAR * (r * c - VAR(_c2));
        FPy += VVAR * r * s;
      } else {
        FPx += VVAR * FTx;
        FPy += VVAR * FTy;
      ...
      double r = sqrt(sqr(-VAR(_c2) - FTx) + sqr(FTy));

      if (r <= VAR(r2)) {
        r *= VAR(r1) / VAR(r2);
        double a = atan2(FTy, -VAR(_c2) - FTx);
        double s = sin(a);
        double c = cos(a);
        FPx += VVAR * (r * c + VAR(_c1));
        FPy += VVAR * r * s;
      } else {
        FPx += VVAR * FTx;
        FPy += VVAR * FTy;
