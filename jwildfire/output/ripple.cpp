/*
    Apophysis Plugin: ripple

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/RippleFunc.java
...
APO_PLUGIN("ripple");
APO_VARIABLES(
	VAR_REAL(frequency, 2.0),
	VAR_REAL(velocity, 1.0),
...
double lerp (Variation* vp, double a, double b, double p) {
...
int PluginVarPrepare(Variation* vp)
{
    ...
    // - transform them
    VAR(_f) = VAR(frequency) * 5;
    double a = VAR(amplitude) * 0.01;
    VAR(_p) = VAR(phase) * M_2PI - M_PI;

    // VAR(scale) must not be zero
    VAR(_s) = VAR(scale) == 0 ? SMALL_EPSILON : VAR(scale);

    // we will need the inverse VAR(scale)
    VAR(_is) = 1.0 / VAR(_s);

    // pre-multiply VAR(velocity)+VAR(phase), VAR(phase)+VAR(amplitude) and (PI-VAR(phase))+VAR(amplitude)
    VAR(_vxp) = VAR(velocity) * VAR(_p);
    VAR(_pxa) = VAR(_p) * a;
    VAR(_pixa) = (M_PI - VAR(_p)) * a;
    VAR(_fixed_dist_calc) = VAR(fixed_dist_calc) != 0;
...
int PluginVarCalc(Variation* vp)
{
    ...
    //align input x, y to given center and multiply with VAR(scale)
    double x = (FTx * VAR(_s)) - VAR(centerx), y = (FTy * VAR(_s)) + VAR(centery);

    // calculate distance from center but constrain it to EPS
    double d = VAR(_fixed_dist_calc) ? sqrt(x * x + y * y) : sqrt(x * x * y * y);
    if (d < SMALL_EPSILON)
      d = SMALL_EPSILON;
    ...
    // calculate cosine wave with given VAR(frequency), VAR(velocity) 
    // and VAR(phase) based on the distance to center
    double wave = cos(VAR(_f) * d - VAR(_vxp));

    // calculate the wave offsets
    double d1 = wave * VAR(_pxa) + d, d2 = wave * VAR(_pixa) + d;

    // we got two offsets, so we also got two new positions (u,v)
    double u1 = (VAR(centerx) + nx * d1), v1 = (-VAR(centery) + ny * d1);
    double u2 = (VAR(centerx) + nx * d2), v2 = (-VAR(centery) + ny * d2);

    // interpolate the two positions by the given VAR(phase) and
    // invert the multiplication with VAR(scale) from before
    FPx = VVAR * (lerp(vp, u1, u2, VAR(_p))) * VAR(_is);
    FPy = VVAR * (lerp(vp, v1, v2, VAR(_p))) * VAR(_is);
