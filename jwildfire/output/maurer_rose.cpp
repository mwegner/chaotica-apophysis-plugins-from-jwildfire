/*
    Apophysis Plugin: maurer_rose

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/MaurerRoseFunc.java
...
typedef struct
{
	double kn; // numerator of k in rhodonea curve equation,   k = kn/kd
	double kd; // denominator of k in rhodonea curve equation, k = kn/kd
	double _k;     // for rhodonea equation, r = cos(k * theta)
	double c; // offset for extended rhodonea curve equation, r = cos(k * theta) + c
	double _cycles; // 1 cycle = 2*PI
	double line_count;
	double line_offset_degrees;
	double _step_size_radians;
	double show_lines;
	double show_points;
	double show_curve;
	double _line_fraction;
	double _point_fraction;
	double _curve_fraction;
	double _line_threshold;
	double _point_threshold;
	double _point_half_threshold;
...
APO_PLUGIN("maurer_rose");
APO_VARIABLES(
	VAR_REAL(kn, 2),
	VAR_REAL(kd, 1),
	VAR_REAL(c, 0),
	VAR_REAL(line_count, 360),
	VAR_REAL(line_offset_degrees, 71),
...
void getCurveCoords (Variation* vp, double theta, DoublePoint2D result) {

    double r = cos(VAR(_k) * theta) + VAR(c);
...
int PluginVarPrepare(Variation* vp)
{
    GOODRAND_PREPARE();
    // rhodonea init   
    VAR(_k) = VAR(kn) / VAR(kd);
    VAR(_step_size_radians) = M_2PI * (VAR(line_offset_degrees) / 360);
    VAR(_cycles) = (VAR(line_count) * VAR(_step_size_radians)) / M_2PI;

    // line/curve/point rendering init
    double show_sum = VAR(show_lines) + VAR(show_points) + VAR(show_curve);
    VAR(_line_fraction) = VAR(show_lines) / show_sum;
    VAR(_point_fraction) = VAR(show_points) / show_sum;
    VAR(_curve_fraction) = VAR(show_curve) / show_sum;
    VAR(_line_threshold) = VAR(_line_fraction);
    VAR(_point_threshold) = VAR(_line_fraction) + VAR(_point_fraction);
    VAR(_point_half_threshold) = VAR(_line_fraction) + (VAR(_point_fraction) / 2);
...
int PluginVarCalc(Variation* vp)
{
    ...
    double t = VAR(_cycles) * tin; // angle of rose curve

    // map to a Maurer Rose line
    // find nearest step
    double step_number = floor(t / VAR(_step_size_radians));

    // find coordinates for endpoints (on rhodonea curve) for nearest Maurer Rose line
    double theta1 = step_number * VAR(_step_size_radians);
    double theta2 = theta1 + VAR(_step_size_radians);
    getCurveCoords(vp, theta1, endpoint1);
    double x1 = endpoint1.x;
    double y1 = endpoint1.y;
    getCurveCoords(vp, theta2, endpoint2);
    ...
    if (rnd < VAR(_line_threshold)) {
      ...
      if (rnd <= VAR(_point_half_threshold)) {
        xout = x1 + xoffset;
        yout = y1 + yoffset;
      } else {
        xout = x2 + xoffset;
      ...
      // draw rhodonea curve
      getCurveCoords(vp, t, curve_point);
      double curvex = curve_point.x;
      double curvey = curve_point.y;
      if (VAR(_curve_thickness) != 0) {
        xout = curvex + ((GOODRAND_01() - 0.5) * VAR(_curve_thickness));
        yout = curvey + ((GOODRAND_01() - 0.5) * VAR(_curve_thickness));
      } else {
        xout = curvex;
        yout = curvey;
      }
