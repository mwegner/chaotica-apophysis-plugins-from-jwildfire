/*
    Apophysis Plugin: shredlin

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/ShredlinFunc.java
...
{
    // Shredlin by Zy0rg
    double sxd = VAR(xdistance);
    double sxw = VAR(xwidth);
    double syd = VAR(ydistance);
    double syw = VAR(ywidth);
    double vv = VVAR;
    ...
    FPx = vv * sxd * ((xrng - (int) xrng) * sxw + (int) xrng + (0.5 - xpos) * (1 - sxw));
    FPy = vv * syd * ((yrng - (int) yrng) * syw + (int) yrng + (0.5 - ypos) * (1 - syw));
