...
/*
    Apophysis Plugin: hexaplay3D

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Hexaplay3DFunc.java
...
typedef struct
{
	...
	double _seg120x[3];
	double _seg120y[3];
	int _rswtch; //  A value for choosing between 6 or 3 segments to a plane
	int _fcycle; //  markers to count cycles...
	int _bcycle;
	double majp; // establishes 1 or 2 planes, and if 2, the distance between them
	double scale; // scales the effect of X and Y
	double zlift; // scales the effect of Z axis within the snowflake

} Variables;

#define APO_VARIABLE_PREFIX "hexaplay3D_"
#include "plugin.h"
