...
/*
    Apophysis Plugin: dc_circuits

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/DC_CircuitsFunc.java
...
typedef struct
{
	double _w; // (intensity * zoom) * M
	double _circ; // Math.pow(G.max(0.0
	double _w - ot2) / w;
	double _6.0);

} Variables;

#define APO_VARIABLE_PREFIX "dc_circuits_"
#include "plugin.h"

APO_PLUGIN("dc_circuits");
APO_VARIABLES(
