...
/*
    Apophysis Plugin: dc_carpet3D

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/DCCarpet3DFunc.java
...
typedef struct
{
	int _x0; // (pContext.random() < 0.5) ? -1 : 1
	int _y0; // (pContext.random() > 0.5) ? -1 : 1
	double _x = 0.0; // pAffineTP.x + x0 * stretch_x
	double _y = 0.0; // pAffineTP.y + y0 * stretch_y
	double _x0_xor_y0; // (double) (x0 ^ y0)
	double _hh; // -H + (color_b - x0_xor_y0) * H
	double _dz = 0.0; // pVarTP.color * scale_z + offset_z

} Variables;

#define APO_VARIABLE_PREFIX "dc_carpet3D_"
#include "plugin.h"

APO_PLUGIN("dc_carpet3D");
APO_VARIABLES(
