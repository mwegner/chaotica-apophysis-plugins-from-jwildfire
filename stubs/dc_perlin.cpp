...
/*
    Apophysis Plugin: dc_perlin

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/DCPerlinFunc.java
...
typedef struct
{
	...
	double z;
	int select_bailout;
	double _notch_bottom;
	double _notch_top;

} Variables;

#define APO_VARIABLE_PREFIX "dc_perlin_"
#include "plugin.h"

APO_PLUGIN("dc_perlin");
APO_VARIABLES(
	VAR_INTEGER(shape, 0),
	VAR_INTEGER(map, 0),
	VAR_REAL(select_centre, 0.0),
	VAR_REAL(select_range, 1.0),
	VAR_REAL(centre, 0.25),
	VAR_REAL(range, 0.25),
	VAR_REAL(edge, 0.0),
	VAR_REAL(scale, 1.0),
	VAR_INTEGER(octaves, 2),
	VAR_REAL(amps, 2.0),
	VAR_REAL(freqs, 2.0),
	VAR_REAL(z, 0.0),
