...
double distribute(Variation* vp, double a, double min, double max) {
    double distance = random01() * 0.5 * VAR(A);
    return a < min ? min + distance * (max - min) : max - distance * (max - min);
}
...
int PluginVarCalc(Variation* vp)
{
    FPx = VVAR * ((FPx >= VAR(xmin)) && (FPx <= VAR(xmax)) ? FPx :
          distribute(vp, FPx, VAR(xmin), VAR(xmax)));
    FPy = VVAR * ((FPy >= VAR(ymin)) && (FPy <= VAR(ymax)) ? FPy :
          distribute(vp, FPy, VAR(ymin), VAR(ymax)));
