...
/*
    Apophysis Plugin: sunflower

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/SunFlowersFunc.java
...
APO_PLUGIN("sunflower");
APO_VARIABLES(
	VAR_INTEGER(nPoints, 500),
	VAR_INTEGER(shape, 10),
	VAR_REAL(scale, 0.02),
...
void build_sunflower (Variation* vp) {
    ...
    for (int i = 0; i < VAR(nPoints); i++) {
      ...
      if (VAR(invert) == 1)
        sc = r / rmax;
      //primitives.add(new Ngon(VAR(shape),VAR(scale)*sc,0.0,new Point(x,y),(double)i/VAR(nPoints),VAR(_fill)));
      primitives.add(new Ngon(VAR(shape), VAR(scale) * sc, 0.0, new Point(x, y), sc, VAR(_fill)));
    }
}
