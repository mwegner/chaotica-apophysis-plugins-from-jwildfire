...
/*
    Apophysis Plugin: juliascope

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/JuliaScopeFunc.java
...
void transformPower2 (Variation* vp) {
    ...
    if (GOODRAND_0X(2) == 0) {
      ...
      a = M_PI - atan2(FTy, FTx) / 2.0;
    }
    double sina = sin(a);
    double cosa = cos(a);
    double r = VVAR * sqrt(sqrt(sqr(FTx) + sqr(FTy)));
...
void transformFunction (Variation* vp) {
    ...
    int rnd = GOODRAND_0X(VAR(_absPower));
    double a;
    if ((rnd & 1) == 0)
      a = (2 * M_PI * rnd + atan2(FTy, FTx)) / VAR(power);
    else
      a = (2 * M_PI * rnd - atan2(FTy, FTx)) / VAR(power);
    double sina = sin(a);
    double cosa = cos(a);
