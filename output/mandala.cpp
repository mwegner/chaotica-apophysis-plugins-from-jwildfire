...
/*
    Apophysis Plugin: mandala

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/MandalaFunc.java
...
void build_mandala (Variation* vp) {

    bool boolean_minsky = false;
    double wobble, wrap_range;
    int extra_hskew;
    GOODRAND_01();

    RotatorMap rm = new RotatorMap(VAR(_width), VAR(_height), VAR(_colorid));
    if (VAR(_minsky) == 1)
      boolean_minsky = true;

    wobble = VAR(_wobble_dat)[VAR(_wobble_pick)];
    if (VAR(_wobble_pick) == 0)
      wobble = 0.0;
    else
      wobble = pow(10.0, -6.0 + .5 * VAR(_wobble_pick));

    wrap_range = VAR(_wrap_dat)[VAR(_wrap_range_pick)];
    extra_hskew = skew_dat[VAR(_extra_hskew_pick)];

    rm.drawMap(primitives, VAR(_num), VAR(_denom), boolean_minsky, wobble, wrap_range, extra_hskew);
}
...
int PluginVarCalc(Variation* vp)
{
    ...
    if (primitive.gettype() == 4) {
      ...
      FPx += 0.01 * VVAR * (out.getX() * VAR(_size) * polygon.getCosa() + out.getY() * VAR(_size) * polygon.getSina() + polygon.getPos().getX() - VAR(_width) / 2);
      FPy += 0.01 * VVAR * (-out.getX() * VAR(_size) * polygon.getSina() + out.getY() * VAR(_size) * polygon.getCosa() + polygon.getPos().getY() - VAR(_width) / 2);
