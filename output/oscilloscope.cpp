...
{
    VAR(_tpf) = 2.0f * M_PI * VAR(frequency);
...
int PluginVarCalc(Variation* vp)
{
    ...
    double t;
    if (VAR(_noDamping)) {
      t = VAR(amplitude) * cos(VAR(_tpf) * FTx) + VAR(separation);
    } else {
      t = VAR(amplitude) * exp(-fabs(FTx) * VAR(damping)) * cos(VAR(_tpf) * FTx) + VAR(separation);
    }
