...
{
    VAR(_absN) = fabs((double) VAR(power));
    VAR(_cN) = (VAR(dist) / VAR(power) - 1.0) / 2.0;
...
int PluginVarCalc(Variation* vp)
{
    ...
    double z = FTx / VAR(_absN);
    double radius_square = FTx * FTx + FTy * FTy;

    double radius_out = VVAR * pow(radius_square + z * z, VAR(_cN));
    ...
    double x = c00 * FTx + c01 * FTy + c20;
    double y = c10 * FTx + c11 * FTy + c21;

    double rand = (int) (GOODRAND_01() * VAR(_absN));

    double alpha = (atan2(y, x) + M_2PI * rand) / VAR(power);
