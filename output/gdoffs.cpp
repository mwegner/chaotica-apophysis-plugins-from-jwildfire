...
/*
    Apophysis Plugin: gdoffs

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/GDOffsFunc.java
...
APO_PLUGIN("gdoffs");
APO_VARIABLES(
	VAR_REAL(delta_x, 0.0),
	VAR_REAL(delta_y, 0.0),
	VAR_REAL(area_x, 2.0),
	VAR_REAL(area_y, 2.0),
	VAR_REAL(center_x, 0),
	VAR_REAL(center_y, 0),
...
double fclp (Variation* vp, double a) {

    return ((a < 0) ? -(fmod(fabs(a), 1)) : fmod(fabs(a), 1));
}

double fscl (Variation* vp, double a) {

    return fclp(vp, (a + 1) / 2);
}

double fosc (Variation* vp, double p, double a) {

    return fscl(vp, -1 * cos(p * a * M_2PI));
}

double flip (Variation* vp, double a, double b, double c) {
...
{
    VAR(_gdodx) = VAR(delta_x) * __agdod;
    VAR(_gdody) = VAR(delta_y) * __agdod;

    VAR(_gdoax) = ((fabs(VAR(area_x)) < 0.1) ? 0.1 : fabs(VAR(area_x))) * __agdoa;
    VAR(_gdoay) = ((fabs(VAR(area_y)) < 0.1) ? 0.1 : fabs(VAR(area_y))) * __agdoa;

    VAR(_gdocx) = (VAR(center_x)) * __agdoc;
    VAR(_gdocy) = (VAR(center_y)) * __agdoc;

    VAR(_gdog) = VAR(gamma);
    VAR(_gdos) = VAR(square);

    VAR(_gdob) = (double) (VAR(_gdog)) * __agdoa / ( MAX(VAR(_gdoax), VAR(_gdoay)));
...
int PluginVarCalc(Variation* vp)
{
    ...
    double osc_x = fosc(vp, VAR(_gdodx), 1), osc_y = fosc(vp, VAR(_gdody), 1);
    double in_x = (FTx + VAR(_gdocx)), in_y = (FTy + VAR(_gdocy));
    double out_x = 0, out_y = 0;

    if (VAR(_gdos) != 0) {
      out_x = flip(vp, flip(vp, in_x, fosc(vp, in_x, 4), osc_x), fosc(vp, fclp(vp, VAR(_gdob) * in_x), 4), osc_x);
      out_y = flip(vp, flip(vp, in_y, fosc(vp, in_y, 4), osc_x), fosc(vp, fclp(vp, VAR(_gdob) * in_y), 4), osc_x);
    } else {
      out_x = flip(vp, flip(vp, in_x, fosc(vp, in_x, 4), osc_x), fosc(vp, fclp(vp, VAR(_gdob) * in_x), 4), osc_x);
      out_y = flip(vp, flip(vp, in_y, fosc(vp, in_y, 4), osc_y), fosc(vp, fclp(vp, VAR(_gdob) * in_y), 4), osc_y);
    }
