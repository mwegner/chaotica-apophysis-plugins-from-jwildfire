...
/*
    Apophysis Plugin: waves2_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Waves2WFFunc.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    if (VAR(use_cos_x) == 1) {
      FPx += VVAR * (FTx + VAR(_dampingX) * VAR(scalex) * cos(FTy * VAR(freqx))) * VAR(_dampingX);
    } else {
      FPx += VVAR * (FTx + VAR(_dampingX) * VAR(scalex) * sin(FTy * VAR(freqx))) * VAR(_dampingX);
    }
    if (VAR(use_cos_y) == 1) {
      FPy += VVAR * (FTy + VAR(_dampingY) * VAR(scaley) * cos(FTx * VAR(freqy))) * VAR(_dampingY);
    } else {
      FPy += VVAR * (FTy + VAR(_dampingY) * VAR(scaley) * sin(FTx * VAR(freqy))) * VAR(_dampingY);
    }
