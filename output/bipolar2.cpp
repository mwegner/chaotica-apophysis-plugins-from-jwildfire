...
/*
    Apophysis Plugin: bipolar2

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Bipolar2Func.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    double x2y2 = (FTx * FTx + FTy * FTy) * VAR(g1);
    double t = x2y2 + VAR(a);
    double x2 = VAR(b) * FTx;
    double ps = -M_PI_2 * VAR(shift);
    double y = VAR(c) * atan2(VAR(e) * FTy, x2y2 - VAR(d)) + ps;
    ...
    if ((g == 0) || (f / g <= 0))
      return TRUE;
    FPx += VVAR * VAR(f1) * M_2_PI * log((t + x2) / (t - x2));
    FPy += VVAR * M_2_PI * y * VAR(h);
