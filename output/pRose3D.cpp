...
/*
    Apophysis Plugin: pRose3D

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/PRose3DFunc.java
...
{
	double l;
	double k; // 3.0 + (random01() < 0.5 ? random01() * 10.0 : Tools.randomInt(15))
	double c;
	double z1;
	double z2;
	double refSc;
...
#define APO_VARIABLE_PREFIX "pRose3D_"
#include "plugin.h"
...
int PluginVarPrepare(Variation* vp)
{
    ...
    if (VAR(k) == 0.0) {
      VAR(_cycle) = M_2PI / (VAR(k) + 0.000001); // describes the segment of a full circle used for each petal
    } else {
      VAR(_cycle) = M_2PI / (VAR(k)); // describes the segment of a full circle used for each petal
    }
    VAR(_optDir) = 0.0;
    VAR(_petalsSign) = 0.0;
...
int PluginVarCalc(Variation* vp)
{
    ...
    int posNeg = 1;
    double th = 0.0;
    double sth, cth, pang, wig, wag, wag2, wag3, wag12 = 0.0, waggle = 0.0, ghostPrep; // ?? VAR(offset) as variable? or preset calculated value?
    double length = VAR(l);
    double numPetals = VAR(k);
    double constant = VAR(c);
    double scaleZ1 = VAR(z1);
    double scaleZ2 = VAR(z2);
    double smooth12 = fabs(VAR(opt));
    double smooth3 = fabs(VAR(opt3));
    double opScale = fabs(VAR(optSc));
    double ghost = sqr(VAR(transp)); // Sets the testing limit for randoms to determine transparency
    double wagScale = VAR(wagsc);
    double curveScale = VAR(srvsc);
    double frequency = VAR(f) * M_2PI;
    double wigScale = VAR(wigsc) * 0.5;
    ...
    ghostPrep = pContext.getRandGen()GOODRAND_01();
    th = atan2(FTy, FTx); // returns the angle this point has with the axis
    sth = sin(th);
    cth = cos(th);
    VAR(_optDir) = Math.copySign(1.0, VAR(opt));
    VAR(_petalsSign) = Math.copySign(1.0, VAR(k));
    ...
    if (fabs(numPetals) < 0.0001) {
      numPetals = 0.0001 * VAR(_petalsSign); // need a non-zero minimum
    }

    if (pContext.getRandGen()GOODRAND_01() < 0.5) {
      posNeg = -1;
    }

    if (VAR(_cycle) == 0.0) {
      pang = th / VAR(_cycle) + EPSILON; // point's angle location relative to which petal it belongs to
    } else {
      pang = th / VAR(_cycle); // point's angle location relative to which petal it belongs to
    }

    // circumference relative VAR(offset), with high frequency values, this causes the ripples to progress along the perimeter
    wig = pang * frequency * 0.5 + VAR(offset) * VAR(_cycle);
    ...
    */
    ...
    //FPx += VVAR*0.5*(length*cos(numPetals*th+constant))*cth; 
    //FPy += VVAR*0.5*(length*cos(numPetals*th+constant))*sth; 
    ...
    if (ghostPrep < ghost) // occasion to draw reflection... but only when posNeg is negative
    {
      ...
      {
        FPx += VVAR * 0.5 * VAR(refSc) * (length * cos(numPetals * th + constant)) * cth;
        FPy += VVAR * 0.5 * VAR(refSc) * (length * cos(numPetals * th + constant)) * sth;
        FPz += VVAR * -0.5 * ((scaleZ2 * waggle + sqr(rad * 0.5) * sin(wig) * wigScale) + (VAR(dist))); // adjustable height

      } else {
        FPx += VVAR * 0.5 * (length * cos(numPetals * th + constant)) * cth;
        FPy += VVAR * 0.5 * (length * cos(numPetals * th + constant)) * sth;
        FPz += VVAR * 0.5 * ((scaleZ1 * waggle + sqr(rad * 0.5) * sin(wig) * wigScale) + (VAR(dist)));
      }
