...
/*
    Apophysis Plugin: rhodonea

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/RhodoneaFunc.java
...
typedef struct
{
	double knumer; // numerator of k in rose curve equations,   k = kn/kd
	double kdenom; // denominator of k in rose curve equations, k = kn/kd
	double radial_offset; // often called "c" in rose curve modifier equations
	int inner_mode; // transform mode to use for incoming points within or on the curve
	int outer_mode; // transform mode to use for incoming points outside the curve
	double inner_spread; // amount to spread away from rose curve within or on the curve, based on incoming point x/y coords
	double outer_spread; // amount to spread away from rose curve outside the curve, based on incoming point x/y coords
	double inner_spread_ratio; // how much inner spread applies to x relative to y
	double outer_spread_ratio; // how much outer spread applies to x relative to y
	double spread_split; // scaling of input point radius to determine inner/outer threshold
	double cycles; // number of cycles (roughly circle loops?), if set to 0 then number of cycles is calculated automatically to close curve (if possible)
	double cycle_offset; // radians to offset cycle for incoming points
	double metacycle_expansion; // expansion factor for each metacycle
	double metacycles; // if cycles is calculated automatically to close the curve, metacycles is number of times to loop over closed curve
	double fill; // amount to thicken curve by randomizing input
	double _kn;
	double _kd;
	double _k; // k = kn/kd
...
APO_VARIABLES(
	VAR_REAL(knumer, 3),
	VAR_REAL(kdenom, 4),
	VAR_REAL(radial_offset, 0),
	VAR_INTEGER(inner_mode, 1),
	VAR_INTEGER(outer_mode, 1),
	VAR_REAL(inner_spread, 0),
	VAR_REAL(outer_spread, 0),
...
int PluginVarPrepare(Variation* vp)
{
    VAR(_kn) = VAR(knumer);
    VAR(_kd) = VAR(kdenom);
    VAR(_k) = VAR(_kn) / VAR(_kd);

    // attempt to calculate minimum VAR(_cycles) manually, or reasonable upper bound if unsure
    if ((VAR(_k) % 1) == 0) { // VAR(_k) is integer 
      if ((VAR(_k) % 2) == 0) { // VAR(_k) is even integer, will have 2k petals
        VAR(_cycles_to_close) = 1; // (2PI)
      } else { // VAR(_k) is odd integer, will have VAR(_k) petals (or sometimes 2k with offset)
        if (VAR(radial_offset) != 0 || VAR(inner_spread) != 0 || VAR(outer_spread) != 0 || VAR(fill) != 0) {
          VAR(_cycles_to_close) = 1;
        } // if adding an offset or spread, need a full cycle
        else {
          VAR(_cycles_to_close) = 0.5;
        } // (1PI)
      }
    } else if ((VAR(_kn) % 1 == 0) && (VAR(_kd) % 1 == 0)) {
      // if VAR(_kn) and VAR(_kd) are integers,
      //   determine if VAR(_kn) and VAR(_kd) are relatively prime (their greatest common denominator is 1)
      //   using builtin gcd() function for BigIntegers in Java
      // and if they're not, make them
      BigInteger bigkn = BigInteger.valueOf((long) VAR(_kn));
      BigInteger bigkd = BigInteger.valueOf((int) VAR(_kd));
      ...
      //    If VAR(_kn) and VAR(_kd) are relatively prime, then the rose consists of 2*VAR(_kn) petals if either VAR(_kn) or VAR(_kd) are even, and VAR(_kn) petals if both VAR(_kn) and VAR(_kd) are odd
      //
      // paraphrased from http://mathworld.wolfram.com/Rose.html:
      //    If VAR(_k)=VAR(_kn)/VAR(_kd) is a rational number, then the curve closes at a polar angle of theta = PI * VAR(_kd) if (VAR(_kn) * VAR(_kd)) is odd, and 2 * PI * VAR(_kd) if (VAR(_kn) * VAR(_kd)) is even
      if ((VAR(_kn) % 2 == 0) || (VAR(_kd) % 2 == 0)) {
        VAR(_cycles_to_close) = VAR(_kd); // 2 * PI * VAR(_kd)
      } else {
        VAR(_cycles_to_close) = VAR(_kd) / 2; // PI * VAR(_kd)
      }
    }
    ...
    else if (((VAR(_k) * 2) % 1) == 0) { // VAR(_k) is a half-integer (1/2, 3/2, 5/2, etc.), will have 4k petals
      VAR(_cycles_to_close) = 2;  // (4PI)
      petal_count = 4*VAR(_k);
    }
    */
    /* 
    // superceded by relative prime conditional?
    // If VAR(_k) can be expressed as VAR(_kn)/3, where n is an integer not divisible by 3, the curve will be rose-shaped with n petals if n is odd and 2n petals if n is even.
    //    (case where VAR(_kn) is integer divisible by three is already handled above, where VAR(_k) is integer)
    else if (((VAR(_k) * 3) % 1) == 0) {
      double basekn = VAR(_k) * 3;
      if ((basekn % 2) == 0) {
        VAR(_cycles_to_close) = 3;
        petal_count = 2 * basekn;
      }
      else  {
        VAR(_cycles_to_close) = 1.5;
          petal_count = basekn;
      }
    }
    */
    /*
    // superceded by relative prime conditional????
      If VAR(_k) can be expressed as n±1/6, where n is a nonzero integer, the curve will be rose-shaped with 12k petals.
      else if {
    ...
    */
      ...
      //     if one or both of VAR(_kn) and VAR(_kd) are non-integers, then the above may still be true (VAR(_k) may still be [effectively] rational) but haven't 
      //          figured out a way to determine this.
      //    could restrict VAR(_kn) and VAR(_kd) to integers to simplify, but that would exclude a huge space of interesting patterns
      //    could set VAR(_cycles) extremely high, but if VAR(_k) is truly irrational this will just approarch a uniform distribution across a circle, 
      //                and also exclude a large space of interesting patterns with non-closed curves
      //    so for now keep VAR(_kn) and VAR(_kd) as continuous doubles, and just pick a large but not huge number for VAR(_cycles)
      // 
      //    realistically in this case it is better for user to fiddle with manual VAR(_cycles) setting to get a pattern they like
      //        
      VAR(_cycles_to_close) = 2 * VAR(_kn) * VAR(_kd);
      if (VAR(_cycles) < 16) {
        VAR(_cycles_to_close) = 16;
      } // ???  just want to keep number higher if VAR(_kn) and VAR(_kd) are small but potentially irrational
    }
    if (VAR(cycles) == 0) {
      // use auto-calculation of VAR(_cycles) (2*PI*radians) to close the curve, 
      //     and VAR(metacycles) for how many closed curves to cycle through
      VAR(_cycles) = VAR(_cycles_to_close) * VAR(metacycles);
    } else {
      // manually set number of VAR(_cycles) (VAR(_cycles) are specified in 2*PI*radians)
      VAR(_cycles) = VAR(cycles);
    }
    // System.out.println("VAR(_cycles) to close: " + VAR(_cycles_to_close));
    // System.out.println("VAR(metacycles): " + VAR(metacycles));
    // System.out.println("total VAR(_cycles): " + VAR(_cycles));
...
int PluginVarCalc(Variation* vp)
{
    /*
     ...
     or, alternatively, as a pair of Cartesian parametric equations of the form
        ...
        y = cos(kt)sin(t)
    */
    double rin = VAR(spread_split) * sqrt((FTx * FTx) + (FTy * FTy));
    // atan2 range is [-PI, PI], so tin covers 2PI, or 1 cycle (from -0.5 to 0.5 cycle)
    double tin = atan2(FTy, FTx); // polar coordinate angle (theta in radians) of incoming point
    double t = VAR(_cycles) * (tin + (VAR(cycle_offset) * 2 * M_PI)); // angle of rose curve
    double r = cos(VAR(_k) * t) + VAR(radial_offset); // radius of rose curve 
    // double r = sin(VAR(_k) * t) + VAR(radial_offset);  // can use sin instead of cos to rotate by PI/(2*VAR(_k)) radians
    if (VAR(fill) != 0) {
      r = r + (VAR(fill) * (GOODRAND_01() - 0.5));
    }
    double x = r * cos(t);
    double y = r * sin(t);
    double expansion = floor((VAR(_cycles) * (tin + M_PI)) / (VAR(_cycles_to_close) * 2 * M_PI));
    double adjustedAmount = VVAR + (expansion * VAR(metacycle_expansion));
    double xin, yin;
    double rinx, riny;

    if (abs(rin) > abs(r)) { // incoming point lies outside of current petal of rose curve
      switch (VAR(outer_mode)) {
        case 0: // no spread
          FPx += adjustedAmount * x;
          FPy += adjustedAmount * y;
          break;
        case 1:
          rinx = (rin * VAR(outer_spread) * VAR(outer_spread_ratio)) - (VAR(outer_spread) * VAR(outer_spread_ratio)) + 1;
          riny = (rin * VAR(outer_spread)) - VAR(outer_spread) + 1;
          FPx += adjustedAmount * rinx * x;
          FPy += adjustedAmount * riny * y;
          if (FPy == 0) {
            FPx = 0;
          }
          break;
        case 2:
          ...
          //FPx += adjustedAmount * x + ((rin - r) * VAR(outer_spread) * VAR(outer_spread_ratio));
          FPx += adjustedAmount * (x + (VAR(outer_spread) * VAR(outer_spread_ratio) * (xin - x)));
          FPy += adjustedAmount * (y + (VAR(outer_spread) * (yin - y)));
          break;
        case 3:
          xin = abs(FTx);
          yin = abs(FTy);
          ...
          FPx += adjustedAmount * (x + (VAR(outer_spread) * VAR(outer_spread_ratio) * xin));
          FPy += adjustedAmount * (y + (VAR(outer_spread) * yin));
          break;
        case 4:
          rinx = (0.5 * rin) + (VAR(outer_spread) * VAR(outer_spread_ratio));
          riny = (0.5 * rin) + VAR(outer_spread);
          FPx += adjustedAmount * rinx * x;
          FPy += adjustedAmount * riny * y;
          break;
      ...
      switch (VAR(inner_mode)) {
        case 0: // no spread, all points map to rose curve
          FPx += adjustedAmount * x;
          FPy += adjustedAmount * y;
          break;
        case 1:
          // rin = (rin * VAR(inner_spread)) - VAR(inner_spread) + 1;
          rinx = (rin * VAR(inner_spread) * VAR(inner_spread_ratio)) - (VAR(inner_spread) * VAR(inner_spread_ratio)) + 1;
          riny = (rin * VAR(inner_spread)) - VAR(inner_spread) + 1;
          FPx += adjustedAmount * rinx * x;
          FPy += adjustedAmount * riny * y;
          if (FPy == 0) {
            FPx = 0;
        ...
        case 2:
          ...
          FPx += adjustedAmount * (x - (VAR(inner_spread) * VAR(inner_spread_ratio) * (x - xin)));
          FPy += adjustedAmount * (y - (VAR(inner_spread) * (y - yin)));
          break;
        case 3:
          xin = abs(FTx);
          yin = abs(FTy);
          ...
          FPx += adjustedAmount * (x - (VAR(inner_spread) * VAR(inner_spread_ratio) * xin));
          FPy += adjustedAmount * (y - (VAR(inner_spread) * yin));
          break;
        case 4:
          // rin = (0.5 * rin) + VAR(inner_spread);
          rinx = (0.5 * rin) + (VAR(inner_spread) * VAR(inner_spread_ratio));
          riny = (0.5 * rin) + VAR(inner_spread);
          FPx += adjustedAmount * rinx * x;
          FPy += adjustedAmount * riny * y;
          break;
        case 5: // mask "inside" the curve
        ...
        case 6: // mask "outside" the curve
          ...
          FPy += FTy;
          break;
        default: // shouldn't reach here (VAR(inner_mode) is constrained to be 0-6), but just in case...
          FPx += adjustedAmount * x;
          FPy += adjustedAmount * y;
          break;
      }
    ...
    if (draw_diagnostics) {
      double diagnostic = GOODRAND_01() * 100;
      ...
      if (diagnostic <= 3) { // diagnostic = (0-3]
        ...
        FPx = radius * cos(angle);
        FPy = radius * sin(angle);
      }
      // draw diagnostic unit squares
        ...
        double varpos = (GOODRAND_01() * unit * 2) - unit;
        double sx = 0, sy = 0;
        if (side == 1) {
          sx = unit;
          sy = varpos;
