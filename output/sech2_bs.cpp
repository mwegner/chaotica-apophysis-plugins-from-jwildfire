...
/*
    Apophysis Plugin: sech2_bs

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Sech2_BSFunc.java
...
#define APO_VARIABLE_PREFIX "sech2_bs_"
#include "plugin.h"
...
int PluginVarCalc(Variation* vp)
{
    ...
    double sechsin = sin(FTy * VAR(y1));
    double sechcos = cos(FTy * VAR(y2));
    double sechsinh = sinh(FTx * VAR(x1));
    double sechcosh = cosh(FTx * VAR(x2));
    double d = (cos(2.0 * FTy) + cosh(2.0 * FTx));
    if (d == 0) {
      return TRUE;
    }
    double sechden = 2.0 / d;
    FPx += VVAR * sechden * sechcos * sechcosh;
    FPy -= VVAR * sechden * sechsin * sechsinh;
