...
/*
    Apophysis Plugin: waves4_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Waves4WFFunc.java
...
#define APO_VARIABLE_PREFIX "waves4_wf_"
#include "plugin.h"

APO_PLUGIN("waves4_wf");
APO_VARIABLES(
...
int PluginVarCalc(Variation* vp)
{
    ...
    if (VAR(use_cos_x) == 1) {
      FPx += VVAR * (FTx + VAR(_dampingX) * VAR(scalex) * cos(FTy * VAR(freqx)) * sin(FTy * VAR(freqx)) * cos(FTy * VAR(freqx))) * VAR(_dampingX);
    } else {
      FPx += VVAR * (FTx + VAR(_dampingX) * VAR(scalex) * sin(FTy * VAR(freqx)) * cos(FTy * VAR(freqx)) * sin(FTy * VAR(freqx))) * VAR(_dampingX);
    }
    if (VAR(use_cos_y) == 1) {
      FPy += VVAR * (FTy + VAR(_dampingY) * VAR(scaley) * cos(FTx * VAR(freqy)) * sin(FTx * VAR(freqy)) * cos(FTx * VAR(freqy))) * VAR(_dampingY);
    } else {
      FPy += VVAR * (FTy + VAR(_dampingY) * VAR(scaley) * sin(FTx * VAR(freqy)) * cos(FTx * VAR(freqy)) * sin(FTx * VAR(freqy))) * VAR(_dampingY);
    }
