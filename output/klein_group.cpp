...
/*
    Apophysis Plugin: klein_group

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/KleinGroupFunc.java
...
const int GRANDMA_STANDARD = 0;
const int MASKIT_MU = 1;
const int JORGENSEN = 2;
const int RILEY = 3;
const int RILEY_MODIFIED = 4;
const int MASKIT_MU_MODIFIED = 5;
const int MASKIT_LEYS_MODIFIED = 6;
...
{
    Complex generators[][];
    if (VAR(recipe) == GRANDMA_STANDARD) {
      generators = calcGrandmaGenerators();
    } else if (VAR(recipe) == MASKIT_MU) {
      generators = calcMaskitGenerators();
    } else if (VAR(recipe) == MASKIT_MU_MODIFIED) {
      generators = calcModifiedMaskitGenerators();
    } else if (VAR(recipe) == JORGENSEN) {
      generators = calcJorgensenGenerators();
    } else if (VAR(recipe) == RILEY) {
      generators = calcRileyGenerators();
    } else if (VAR(recipe) == RILEY_MODIFIED) {
      generators = calcModifiedRileyGenerators();
    } else if (VAR(recipe) == MASKIT_LEYS_MODIFIED) {
...
int PluginVarCalc(Variation* vp)
{
    ...
    int mindex = GOODRAND_0X(mtransforms.length);
