...
/*
    Apophysis Plugin: glsl_randomoctree

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/GLSLRandomOctreeFunc.java
...
typedef struct
{
	...
	double _time = 0.0;
	int _steps = 10;
	double _mouseX = 0.0;  // 0-1
	double _mouseY = 0.0;  // 0-1
	int _grid;
	int _borders = 1;
	int _blackborders = 1;
	int _gradient;
	double _emptycells = 0.5;
...
double rnd (Variation* vp, vec4 v) {

    return G.fract(4e4 * G.sin(G.dot(v, new vec4(13.46, 41.74, -73.36, 14.24)) + 17.34));
}

int getvoxel (Variation* vp, vec3 p, double size) {
...
//	#endif
    ...
    double val = rnd(vp, new vec4(p, size));

    if (val < VAR(_emptycells)) {
      return 0;
    } else if (val < VAR(_subdivisions)) {
      return 1;
    } else {
      return 2;
    }
...
double map (Variation* vp, vec3 p, vec3 fp) {
...
double sqr (Variation* vp, double a) {
