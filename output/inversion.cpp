...
/*
    Apophysis Plugin: inversion

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/InversionFunc.java
...
typedef struct
{
	bool _draw_guides = false;
	double rotation;
	double _shape_rotation_radians;
	double _scalesqr;
	double scale;
	double a;
	double b;
	...
	bool guides_enabled;
	int imode; // STANDARD
	bool hide_uninverted;
	int color_gradient; // COLORMAP_CLAMP
	int color_measure; // NONE
...
APO_VARIABLES(
	...
	VAR_REAL(e, 0),
	VAR_REAL(f, 0),
	VAR_INTEGER(shape, CIRCLE),
	VAR_REAL(passthrough, 0),
	VAR_REAL(pnorm_point, 2),
	VAR_REAL(pnorm_shape, 2),
	VAR_REAL(pnorm_pmod, 1),
	VAR_REAL(pnorm_smod, 1),
	VAR_REAL(ring_scale, 1),
	VAR_INTEGER(ring_mode, IGNORE_RING),
	VAR_REAL(draw_shape, 0),
	VAR_REAL(shape_thickness, 0),
	VAR_INTEGER(guides_enabled, 1),
	VAR_INTEGER(imode, STANDARD),
	VAR_INTEGER(hide_uninverted, 1),
...
void setDrawGuides (Variation* vp, boolean draw) {

    VAR(_draw_guides) = draw;
}

boolean getDrawGuides (Variation* vp) {

    return VAR(_draw_guides);
}

void setColor (Variation* vp, XYZPoint srcPoint, XYZPoint dstPoint, PolarPoint2D curvePoint, double pAmount) {

    if (VAR(color_measure) != NONE) {
      double val = 0;
      double[] sampled_vals;
      if (VAR(color_measure) == DST_DISTANCE_FROM_BOUNDARY) {
        double xdiff = dstPoint.x - curvePoint.x;
        double ydiff = dstPoint.y - curvePoint.y;
        val = sqrt((xdiff * xdiff) + (ydiff * ydiff));
      } else if (VAR(color_measure) == SRC_DISTANCE_FROM_BOUNDARY) {
        double xdiff = srcPoint.x - curvePoint.x;
        double ydiff = srcPoint.y - curvePoint.y;
        val = sqrt((xdiff * xdiff) + (ydiff * ydiff));
      } else if (VAR(color_measure) == DST_DISTANCE_FROM_SRC) {
        double xdiff = dstPoint.x - srcPoint.x;
        double ydiff = dstPoint.y - srcPoint.y;
        val = sqrt((xdiff * xdiff) + (ydiff * ydiff));
      } else if (VAR(color_measure) == DST_RADIAL_DIFFERENCE) {
        ...
        double iscale = (VAR(scale) * VAR(scale)) / (sqr(srcPoint.x) + sqr(srcPoint.y));
        double cx = iscale * srcPoint.x * VVAR;
        double cy = iscale * srcPoint.y * VVAR;
      ...
      if (VAR(color_gradient) == COLORMAP_WRAP) {
...
{
   VAR(_draw_guides) = false;
    VAR(_shape_rotation_radians) = M_PI * VAR(rotation);
    VAR(_scalesqr) = VAR(scale) * VAR(scale);

    if (VAR(shape) == CIRCLE) {
      shape = new Circle();
    } else if (VAR(shape) == ELLIPSE) {
      shape = new Ellipse();
    } else if (VAR(shape) == HYPERBOLA) {
      shape = new Hyperbola();
    } else if (VAR(shape) == REGULAR_POLYGON) {
      shape = new RegularPolygon();
    } else if (VAR(shape) == RHODONEA) {
      shape = new Rhodonea();
    } else if (VAR(shape) == SUPERSHAPE) {
      shape = new SuperShape();
    } else if (VAR(shape) == SUPERELLIPSE) {
      shape = new SuperEllipse();
    } else {  // if VAR(shape) not recognized, default to Circle
      shape = new Circle();
    }
    if (VAR(ring_scale) <= VAR(scale)) {
      VAR(_ring_rmin) = VAR(ring_scale) * VAR(scale);
      VAR(_ring_rmax) = (VAR(scale) * VAR(scale)) / VAR(_ring_rmin);
    } else {
      VAR(_ring_rmax) = VAR(ring_scale) * VAR(scale);
      VAR(_ring_rmin) = (VAR(scale) * VAR(scale)) / VAR(_ring_rmax);
    }
...
int PluginVarCalc(Variation* vp)
{
    ...
    double iscale;
    if (VAR(_draw_guides) && VAR(guides_enabled)) {
      double rnd = GOODRAND_01();
      double theta = rnd * shape.getPeriod();
      double split = GOODRAND_01() * 1.1;
      if (split < 1) {
        if (VAR(ring_mode) == IGNORE_RING) {
          ...
          if (split < 0.5) {
            ...
            // draw VAR(_ring_rmin)
            FPx += VAR(_ring_rmin) * cos(theta);
            FPy += VAR(_ring_rmin) * sin(theta);
          } else {
            FPx += VAR(_ring_rmax) * cos(theta);
            FPy += VAR(_ring_rmax) * sin(theta);
            // draw VAR(_ring_rmax)
          }
        }
      } else {
    ...
    if (VAR(draw_shape) > 0) {
      double rnd = GOODRAND_01();
      if (rnd < VAR(draw_shape)) {
        double theta = GOODRAND_01() * shape.getPeriod();
        shape.getCurvePoint(theta, curve_point);
        FPx += curve_point.x;
        FPy += curve_point.y;
        if (VAR(shape_thickness) != 0) {
          FPx += 0.01 * (GOODRAND_01() - 0.5) * VAR(shape_thickness);
          FPy += 0.01 * (GOODRAND_01() - 0.5) * VAR(shape_thickness);
        }
        return TRUE;
      }
    }
    if (VAR(passthrough) > 0) {
      double rnd = GOODRAND_01();
      if (rnd < VAR(passthrough)) {
        FPx += xin;
        FPy += yin;
        return TRUE;
      }
    ...
    if (VAR(imode) == EXTERNAL_INVERSION_ONLY) {
      // only do inversion if input point is outside of circle
      do_inversion = rin > rcurve;
    } else if (VAR(imode) == INTERNAL_INVERSION_ONLY) {
    ...
    //   if inside of curve radius, make sure is > ring_min (based on ring VAR(scale))
    //   if outside of curve radius, make sure is < ring_max (based on inversion of ring_min)
    if (do_inversion && (VAR(ring_mode) != IGNORE_RING) && (VAR(ring_scale) != 1)) {
      if (VAR(ring_mode) == INVERSION_INSIDE_RING_ONLY) {
        do_inversion = (rin >= VAR(_ring_rmin)) && (rin <= VAR(_ring_rmax));
      } else if (VAR(ring_mode) == INVERSION_OUTSIDE_RING_ONLY) {
        do_inversion = (rin <= VAR(_ring_rmin)) || (rin >= VAR(_ring_rmax));
      }
    }
    ...
    if (do_inversion) {
      ...
      double denom_scale;
      if (VAR(pnorm_shape) == 2 && VAR(pnorm_smod) == 1) {
        num_scale = sqr(rcurve);
      } else {
        num_scale = pow((pow(abs(xcurve), VAR(pnorm_shape)) + pow(abs(ycurve), VAR(pnorm_shape))), 2.0 / (VAR(pnorm_shape) * VAR(pnorm_smod)));
      }
      if (VAR(pnorm_point) == 2 && VAR(pnorm_pmod) == 1) {
        denom_scale = sqr(xin) + sqr(yin);
      } else {
        denom_scale = pow((pow(abs(xin), VAR(pnorm_point)) + pow(abs(yin), VAR(pnorm_point))), 2.0 / (VAR(pnorm_point) * VAR(pnorm_pmod)));
      }
      ...
      /* pVarTP.doHide = false; */
      setColor(vp, srcPoint, dstPoint, curve_point, VVAR);
      ...
      pVarTP.doHide = VAR(hide_uninverted);
    }
