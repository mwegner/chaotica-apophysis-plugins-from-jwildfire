...
int rand (Variation* vp) {

    return GOODRAND_0X(0xfffffff);
}
...
int PluginVarCalc(Variation* vp)
{
    ...
    if ((((rand(vp) ^ (rand(vp) << 15)) & 0xfffffff) / (double) 0xfffffff) < VAR(root_face)) {
      FPz = (VVAR < 0 ? 0 : VVAR);
    } else {
      FPz = VVAR * (((rand(vp) ^ (rand(vp) << 15)) & 0xfffffff) / (double) 0xfffffff);
    }
