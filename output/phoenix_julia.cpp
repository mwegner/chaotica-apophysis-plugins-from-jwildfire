...
/*
    Apophysis Plugin: phoenix_julia

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/PhoenixJuliaFunc.java
...
typedef struct
{
	double power; // genRandomPower()
	double dist;
	double x_distort; // -0.5
	double y_distort;
	double _invN;
	double _inv2PI_N;
	double _cN;
...
#define APO_VARIABLE_PREFIX "phoenix_julia_"
#include "plugin.h"

APO_PLUGIN("phoenix_julia");
APO_VARIABLES(
	VAR_REAL(power, 0),
	VAR_REAL(dist, 1.0),
...
{
    VAR(_invN) = VAR(dist) / VAR(power);
    VAR(_inv2PI_N) = M_2PI / VAR(power);
    VAR(_cN) = VAR(dist) / VAR(power) / 2.0;
...
int PluginVarCalc(Variation* vp)
{
    ...
    double preX = FTx * (VAR(x_distort) + 1.0);
    double preY = FTy * (VAR(y_distort) + 1.0);

    double a = atan2(preY, preX) * VAR(_invN) + GOODRAND_0X(INT_MAX) * VAR(_inv2PI_N);
    double sina = sin(a);
    double cosa = cos(a);
    double r = VVAR * pow(sqr(FTx) + sqr(FTy), VAR(_cN));
