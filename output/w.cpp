...
{
    VAR(_hypergon_d) = sqrt(1.0 + sqr(VAR(hypergon_r)));
    VAR(_hypergon_n) = (double) VAR(hypergon_n);

    VAR(_lituus_a) = -VAR(lituus_a);

    VAR(_star_n) = (double) VAR(star_n);
    VAR(_star_slope) = tan(VAR(star_slope));

    VAR(_super_m) = VAR(super_m) / 4.0;
    VAR(_super_n1) = -1.0 / (VAR(super_n1) + SMALL_EPSILON);
...
int PluginVarCalc(Variation* vp)
{
    ...
    if (VAR(hypergon) != 0.0) {
      temp1 = fmod(fabs(a), M_2PI / VAR(_hypergon_n)) - M_PI / VAR(_hypergon_n);
      temp2 = sqr(tan(temp1)) + 1.0;
      if (temp2 >= sqr(VAR(_hypergon_d))) {
        total += VAR(hypergon);
      } else {
        total += VAR(hypergon) * (VAR(_hypergon_d) - sqrt(sqr(VAR(_hypergon_d)) - temp2)) / sqrt(temp2);
      }
    }
    if (VAR(star) != 0.0) {
      temp1 = tan(fabs(fmod(fabs(a), M_2PI / VAR(_star_n)) - M_PI / VAR(_star_n)));
      total += VAR(star) * sqrt(sqr(VAR(_star_slope)) * (1.0 + sqr(temp1)) / sqr(temp1 + VAR(_star_slope)));
    }
    if (VAR(lituus) != 0.0) {
      total += VAR(lituus) * pow(a / M_PI + 1.0, VAR(_lituus_a));
    }
    if (VAR(super) != 0.0) {
      double ang = a * VAR(_super_m);
      s = sin(ang);
      c = cos(ang);
      total += VAR(super) * pow(pow(fabs(c), VAR(super_n2)) + pow(fabs(s), VAR(super_n3)), VAR(_super_n1));
    }

    if (r <= total) {
      if (VAR(hypergon) != 0.0) {
        temp1 = fmod(fabs(a2), M_2PI / VAR(_hypergon_n)) - M_PI / VAR(_hypergon_n);
        temp2 = sqr(tan(temp1)) + 1.0;
        if (temp2 >= sqr(VAR(_hypergon_d))) {
          total2 += VAR(hypergon);
        } else {
          total2 += VAR(hypergon) * (VAR(_hypergon_d) - sqrt(sqr(VAR(_hypergon_d)) - temp2)) / sqrt(temp2);
        }
      }
      if (VAR(star) != 0.0) {
        temp1 = tan(fabs(fmod(fabs(a2), M_2PI / VAR(_star_n)) - M_PI / VAR(_star_n)));
        total2 += VAR(star) * sqrt(sqr(VAR(_star_slope)) * (1.0 + sqr(temp1)) / sqr(temp1 + VAR(_star_slope)));
      }
      if (VAR(lituus) != 0.0) {
        total2 += VAR(lituus) * pow(a2 / M_PI + 1.0, VAR(_lituus_a));
      }
      if (VAR(super) != 0.0) {
        double ang = a2 * VAR(_super_m);
        s = sin(ang);
        c = cos(ang);
        total2 += VAR(super) * pow(pow(fabs(c), VAR(super_n2)) + pow(fabs(s), VAR(super_n3)), VAR(_super_n1));
      }

      r = VVAR * total2 * r / total;
      s = sin(a2);
      c = cos(a2);
