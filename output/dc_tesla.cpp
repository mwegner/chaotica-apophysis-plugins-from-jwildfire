...
double field21 (Variation* vp, vec3 p, double s) {

	double strength = 20.0 + .03 * log(1.e-6d + G.fract(G.sin(time*10.0) * 4500.0));
	double accum = s*3.0;
	double prev = 0.0;
	double tw = 0.0;
	for (int i = 0; i < levels; ++i) {
		
		double VAR(_mag) = G.dot(p,p)*G.pow(s,1.0*G.abs(G.sin(time/8.0)));
		//remove comment for another effect
		//VAR(_mag)*=dot(p,1.0/p);
		p = G.abs(p).division( VAR(_mag)).add(new vec3(-.9, -.234560*G.abs(G.sin(time)), -1.0));
		double VAR(_w) = G.exp(-(double)i / style );
		accum += VAR(_w) *G.exp(-strength * G.pow(G.abs(VAR(_mag) / prev),1.2));
		tw += VAR(_w)*w;
		prev = VAR(_mag);
	}
//	return G. MAX(0.0,4. * accum / tw - .92);
	return G. MAX(0.0,6. * accum / tw - 2.0);
}
	...
	public vec3 getRGBColor(double xt,double yt)
	{
		...
		col=field21(vp, new vec3(pos,1.0),0.5);
		return new vec3(col);
	}
