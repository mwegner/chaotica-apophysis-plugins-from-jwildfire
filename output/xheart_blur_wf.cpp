...
/*
    Apophysis Plugin: xheart_blur_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/XHeartBlurWFFunc.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    double dx = 2.0 - GOODRAND_01() * 4.0;
    double dy = 2.0 - GOODRAND_01() * 4.0;

    double r2_4 = dx * dx + dy * dy + 4;
    if (r2_4 == 0)
      r2_4 = 1;
    double bx = 4 / r2_4, by = VAR(_rat) / r2_4;
    double x = VAR(_cosa) * (bx * dx) - VAR(_sina) * (by * dy);
    double y = VAR(_sina) * (bx * dx) + VAR(_cosa) * (by * dy);
