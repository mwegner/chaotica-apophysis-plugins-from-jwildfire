...
/*
    Apophysis Plugin: gpattern

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/GPatternFunc.java
...
void build_pattern (Variation* vp) {
    ...
    double polyWidth = VAR(lineheight);
    ...
    if (p.length != 0) {
      ...
      for (double posj = 0; posj <= VAR(width); posj += polyWidth) {
        double y = 0;
        for (int polyCntr = 0; y <= VAR(height); polyCntr++) {
          double x = posj + random(vp, -VAR(randPos), VAR(randPos));
          double r = (float) (polyWidth / 2) + random(vp, -VAR(randSize), VAR(randSize));

          if (VAR(ouline) == 1)
            primitives.add(new Ngon(polys[polyCntr], r, VAR(angle), new Point(x, y), VAR(_outlinecolor), VAR(ouline)));
          if (VAR(fill) == 1) {
            double calccolor = fmod(VAR(_color) + VAR(_speedcolor) * (double) GOODRAND_01(), 1.0);
            primitives.add(new Ngon(polys[polyCntr], r, VAR(angle), new Point(x, y), calccolor, 0.0));
          }
...
int PluginVarPrepare(Variation* vp)
{
   VAR(_color) = 0.0;
   VAR(_speedcolor) = 1.0;
   VAR(_outlinecolor) = 0.0;

    VAR(_line_thickness) = 0.5 / 100;
    build_pattern(vp);
...
int PluginVarCalc(Variation* vp)
{
    ...
    if (primitive.gettype() == 2) {
      ...
      VAR(_color) = line.getColor();
      FPx += VVAR * out.getX();
      FPy += VVAR * out.getY();
      TC = VAR(_color);
    } else if (primitive.gettype() == 3) {
      Triangle triangle = (Triangle) primitive;
      out = plotTriangle(vp, triangle);
      VAR(_color) = triangle.getColor();
      FPx += VVAR * out.getX();
      FPy += VVAR * out.getY();
      TC = VAR(_color);
      ...
      VAR(_color) = polygon.getColor();

      FPx += out.getX() * polygon.getScale() * polygon.getCosa() + out.getY() * polygon.getScale() * polygon.getSina() + polygon.getPos().getX();
      FPy += -out.getX() * polygon.getScale() * polygon.getSina() + out.getY() * polygon.getScale() * polygon.getCosa() + polygon.getPos().getY();
      TC = VAR(_color);
    }
