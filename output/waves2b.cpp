...
/*
    Apophysis Plugin: waves2b

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Waves2BFunc.java
...
double safediv (Variation* vp, double q, double r) {
...
double sgn (Variation* vp, double x) {

    // if (x == 0.0) return x;
    return (x < 0 ? -1.0 : 1.0);
}

double Jacobi_elliptic (Variation* vp, double uu, double emmc) {
    ...
    double CA = 0.0003; // (The accuracy is the square of CA.)

    double a, b, c = 0, d = 0, em[13], en[13];
    double dn, cn;
    int bo;
    int l = 0;
    int ii;
...
{
    VAR(_six) = VAR(scalex) - VAR(scaleinfx);
    VAR(_siy) = VAR(scaley) - VAR(scaleinfy);
...
int PluginVarCalc(Variation* vp)
{
    ...
    CsX = safediv(vp, VAR(unity), (VAR(unity) + sqr(FTx)));
    CsX = CsX * VAR(_six) + VAR(scaleinfx);
    CsY = safediv(vp, VAR(unity), (VAR(unity) + sqr(FTy)));
    CsY = CsY * VAR(_siy) + VAR(scaleinfy);

    // x
    if (VAR(pwx) >= 0 && VAR(pwx) < 1e-4) { // jacobi sn
      JCB_sn = Jacobi_elliptic(vp, FTy * VAR(freqx), VAR(jacok));
      FPx += VVAR * (FTx + CsX * JCB_sn);
    } else if (VAR(pwx) < 0 && VAR(pwx) > -1e-4) { // bessel j1
      FPx += VVAR * (FTx + CsX * _j1(FTy * VAR(freqx)));
    } else { // NORMAL!
      FPx += VVAR * (FTx + CsX * sin(sgn(vp, FTy) * pow(fabs(FTy) + 1e-10, VAR(pwx)) * VAR(freqx)));
    }

    // y
    if (VAR(pwy) >= 0 && VAR(pwy) < 1e-4) { // jacobi sn
      JCB_sn = Jacobi_elliptic(vp, FTx * VAR(freqy), VAR(jacok));
      FPy += VVAR * (FTy + CsY * JCB_sn);
    } else if (VAR(pwy) < 0 && VAR(pwy) > -1e-4) { // bessel j1
      FPy += VVAR * (FTy + CsY * _j1(FTx * VAR(freqy)));
    } else { // NORMAL!
      FPy += VVAR * (FTy + CsY * sin(sgn(vp, FTx) * pow(fabs(FTx) + 1e-10, VAR(pwy)) * VAR(freqy)));
    }
