...
int PluginVarPrepare(Variation* vp)
{
    ...
    VAR(_dx) = (VAR(xmax) - VAR(xmin)) / (double) VAR(xcount);
    VAR(_dy) = (VAR(ymax) - VAR(ymin)) / (double) VAR(ycount);
    VAR(_dz) = (VAR(zmax) - VAR(zmin)) / (double) VAR(zcount);
...
int PluginVarCalc(Variation* vp)
{
    ...
    int xIdx = GOODRAND_0X(VAR(xcount));
    int yIdx = GOODRAND_0X(VAR(ycount));
    int zIdx = GOODRAND_0X(VAR(zcount));
    double x = VAR(xmin) + VAR(_dx) * xIdx;
    double y = VAR(ymin) + VAR(_dy) * yIdx;
    double z = VAR(zmin) + VAR(_dz) * zIdx;
    if (VAR(distortion) > 0) {
      long xseed = (VAR(seed) + 1563) * xIdx + zIdx;
      GOODRAND_SEED(xseed);
      double distx = (0.5 - GOODRAND_01()) * VAR(distortion);
      long yseed = (VAR(seed) + 6715) * yIdx + xIdx;
      GOODRAND_SEED(yseed);
      double disty = (0.5 - GOODRAND_01()) * VAR(distortion);
      long zseed = (VAR(seed) + 4761) * zIdx + yIdx;
      GOODRAND_SEED(zseed);
      double distz = (0.5 - GOODRAND_01()) * VAR(distortion);
      x += distx;
      y += disty;
      z += distz;
    }
