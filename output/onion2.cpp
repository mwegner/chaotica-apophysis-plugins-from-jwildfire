...
/*
    Apophysis Plugin: onion2

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Onion2Func.java
...
typedef struct
{
	double _meeting_pt = 0.5;
	double _circle_a = 1.0;
	double _circle_b = 1.0;
	double _shift_x = 0.0;
	double _shift_y = 0.0;
	double _shift_z = 0.0;
...
int PluginVarCalc(Variation* vp)
{
    /* Description:
     ...
     they meet where exp(x) has a slope of 1.0
    ...
    */
    ...
    double t = POINT_SQTR(ini) / VAR(_stretch) - M_PI / 2;

    /* NOTE: "t" and "meetingPt" are in radians */

    if (t > VAR(_meeting_pt)) {
      // exponential curve
      r = cos(t);
      if (tan(VAR(_meeting_pt)) != 0.0) {
        z = exp(cos(VAR(_meeting_pt)) - r) / tan(VAR(_meeting_pt))
                + sin(VAR(_meeting_pt))
                - 1 / tan(VAR(_meeting_pt));

        if (z > VAR(_top_crop) && VAR(_top_crop) > 0) { /* FIX ADDED. VAR(_top_crop) could start at -1 for cropping below middle. */
          z = VAR(_top_crop);
          r = 0;
        }
      } else {
    ...
    r *= VAR(_circle_a) * VVAR;
    z *= VAR(_circle_b) * VVAR;
    ...
    FPx += r * (ini.x / POINT_SQTR(ini));
    FPy += r * (ini.y / POINT_SQTR(ini));
    FPz += z;

    {
      ...
      FPx += r * ini.z * (ini.x / POINT_SQTR(ini));
      FPy += r * ini.z * (ini.y / POINT_SQTR(ini));
