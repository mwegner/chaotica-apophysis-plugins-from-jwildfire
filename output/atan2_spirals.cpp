...
/*
    Apophysis Plugin: atan2_spirals

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Atan2_SpiralsFunc.java
...
typedef struct
{
	double _r_mult = 1.5;
	double _r_add = 1;
	double _xy2_mult = 1.125;
	double _xy2_add = 0.132;
	double _x_mult = 2;
	double _x_add = 0.25;
	double _yx_div = 1;
	double _yx_add;
...
int PluginVarCalc(Variation* vp)
{
    ...
    // final formulas
    double xy2 = pow(xs + ys, VAR(_x2y2_pow));
    double r = pow(xs + ys, VAR(_r_power));

    double fx = VAR(_x_mult) * atan2(r * VAR(_r_mult) + VAR(_r_add), xy2 * VAR(_xy2_mult) + VAR(_xy2_add)) + VAR(_x_add);
    double fy = sin(atan2(y / VAR(_yy_div) + VAR(_yy_add), (x / VAR(_yx_div)) + VAR(_yx_add)) + VAR(_sin_add)) * VAR(_y_mult);
