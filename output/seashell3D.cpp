...
int PluginVarCalc(Variation* vp)
{
     ...
     * parameters
     * VAR(_n): number of Spirals
     * VAR(_a): final Shell Radius
     * VAR(_b): height
     * VAR(_c): inner shell radius
     */
    double t;
    double s;
    t = M_2PI * GOODRAND_01();
    ...
    double x = VAR(_a) * (1 - t / M_2PI) * cos(VAR(_n) * t) * (1.0 + cos(s)) + VAR(_c) * cos(VAR(_n) * t);
    double y = VAR(_a) * (1 - t / M_2PI) * sin(VAR(_n) * t) * (1.0 + cos(s)) + VAR(_c) * sin(VAR(_n) * t);
    double z = VAR(_b) * t / M_2PI + VAR(_a) * (1 - t / M_2PI) * sin(s);
