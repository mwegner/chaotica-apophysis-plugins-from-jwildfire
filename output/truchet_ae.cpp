...
/*
    Apophysis Plugin: truchet_ae

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/TruchetAEFunc.java
...
const double M_SQ3_2 = 0.86602540378443864676372317075294;
...
typedef struct
{
	...
	double step;
	double width;
	double angle; // added Dark
	int layout; // added Dark
	int style; // added Dark
	double K01A;
	double K02A;
	double K01B;
	double K02B;
...
APO_PLUGIN("truchet_ae");
APO_VARIABLES(
	VAR_INTEGER(seed, 0),
	VAR_REAL(step, 0.5),
	VAR_REAL(width, 0.25),
	VAR_REAL(angle, 0.0),
	VAR_INTEGER(layout, 0),
	VAR_INTEGER(style, 0),
	VAR_REAL(K01A, 0.5),
	VAR_REAL(K02A, 0.5),
	VAR_REAL(K01B, 0.5),
...
double fmod2 (Variation* vp, double num, double den) {
...
int PluginVarPrepare(Variation* vp)
{
   ...
   VAR(_z1) = 0.0;
   VAR(_z2) = 0.0;
    VAR(_sinang) = sin(VAR(angle)*M_PI);
    VAR(_cosang) = cos(VAR(angle)*M_PI); 
...
int PluginVarCalc(Variation* vp)
{
    ...
    double u1 = 0, v1 = 0, z = 0.0;
    int i = (int) floor(4 * GOODRAND_01());

    // infinite layouts are possible, here I listed the most famous / interesting / useful
    if (VAR(layout) == 1) { // symmetrized square
      switch (i) {
        case 0:
          u1 = -VAR(_x) * 0.5 + 1.0;
          v1 = VAR(_y) * 0.5 + 1.0;
          VAR(_z1) = fmod2(vp, VAR(K01A) * VAR(_z1) + VAR(K02A), 1.0);
          VAR(_z2) = fmod2(vp, VAR(K01B) * VAR(_z2) + VAR(K02B), 1.0);
          break;
        case 1:
          u1 = VAR(_x) * 0.5 - 1.0;
          v1 = VAR(_y) * 0.5 + 1.0;
          VAR(_z1) = fmod2(vp, VAR(K11A) * VAR(_z1) + VAR(K12A), 1.0);
          VAR(_z2) = fmod2(vp, VAR(K11B) * VAR(_z2) + VAR(K12B), 1.0);
          break;
        case 2:
          u1 = -VAR(_x) * 0.5 + 1.0;
          v1 = -VAR(_y) * 0.5 - 1.0;
          VAR(_z1) = fmod2(vp, VAR(K21A) * VAR(_z1) + VAR(K22A), 1.0);
          VAR(_z2) = fmod2(vp, VAR(K21B) * VAR(_z2) + VAR(K22B), 0.5);
          break;
        case 3:
          u1 = VAR(_x) * 0.5 - 1.0;
          v1 = -VAR(_y) * 0.5 - 1.0;
          VAR(_z1) = fmod2(vp, VAR(K31A) * VAR(_z1) + VAR(K32A), 1.0);
          VAR(_z2) = fmod2(vp, VAR(K31B) * VAR(_z2) + VAR(K32B), 1.0);
          break;
      }
    } else if (VAR(layout) == 2) { // Sierpinski 45°
      switch (i) {
        case 0:
          u1 = VAR(_x) * 0.5 + 1.0;
          v1 = VAR(_y) * 0.5 + 1.0;
    ...
    } else if (VAR(layout) == 4) { // EquilaterTri
      switch (i) {
        case 0:
          u1 = VAR(_x) * 0.5 + 1;
          v1 = VAR(_y) * 0.5 + M_SQ3_2;
          VAR(_z1) = fmod2(vp, VAR(K01A) * VAR(_z1) + VAR(K02A), 1.0);
          VAR(_z2) = fmod2(vp, VAR(K01B) * VAR(_z2) + VAR(K02B), 1.0);
          break;
        case 1:
    ...
    double uu1 = VAR(_cosang) * u1 - VAR(_sinang) * v1;
    double vv1 = VAR(_sinang) * u1 + VAR(_cosang) * v1;
    int m = (int) floor(uu1 / VAR(step));
    int n = (int) floor(vv1 / VAR(step));
    double u = uu1 / VAR(step) - m;
    double v = vv1 / VAR(step) - n;
    double chance = dn1(vp, (m + 34 * n + VAR(seed)));
    double cir = 0.0;
    double square = 0.0;
    bool inv = false;
    ...
    switch (VAR(style)) {
      case 0: // Apo (default) by Eralex
        if (chance > 0.5) {
          if (((v < u + 1 - VAR(width)) && (v > u + VAR(width))) || ((v < u - VAR(width)) && (v > u - 1 + VAR(width)))) z = VAR(_z1);
          else z = VAR(_z2);
        } else {
          if (((v < -u + 1 - VAR(width)) && (v > -u + VAR(width))) || ((v > -u + 1 + VAR(width)) && (v < -u + 2 - VAR(width)))) z = VAR(_z1);
          else z = VAR(_z2);
        }
        break;
      case 1: // Original by Truchet (added VAR(width) for effect) was with four tiles
        square =  MAX(fabs(u - .5), fabs(v - .5)) - 0.5 * (1.0 - VAR(width));
        if (chance < 0.25) {
          if (((square < 0) ^ (u > v))) z = VAR(_z1);
          else z = VAR(_z2);
        } else if (chance < 0.5) {
          if (((square < 0) ^ (u > 1.0 - v))) z = VAR(_z1);
          else z = VAR(_z2);
        } else if (chance < 0.75) {
          if (((square < 0) ^ (u < v))) z = VAR(_z1);
          else z = VAR(_z2);
      ...
      case 2: // circle Apo truchet (simulated) :P
        if (chance < 0.5) {
          cir = sqrt((u - 1.0) * (u - 1.0) + (v - 1.0) * (v - 1.0));
          cir = MIN(cir, sqrt(u * u + v * v));
          cir = fabs(fabs(cir - 0.5) - 0.5) - 0.5 * (1.0 - VAR(width));
          if (cir < 0) z = VAR(_z1);
          else z = VAR(_z2);
        } else {
          cir = sqrt((u - 1.0) * (u - 1.0) + v * v);
          cir = MIN(cir, sqrt(u * u + (v - 1.0) * (v - 1.0)));
          cir = fabs(fabs(cir - 0.5) - 0.5) - 0.5 * (1.0 - VAR(width));
          if (cir < 0) z = VAR(_z1);
          else z = VAR(_z2);
        }
      ...
      case 3: // circle Apo truchet plus + tile - looks cool 8)
        if (chance < 0.333333) {
          ...
          cir = 0.5 * (VAR(width)) - MIN(fabs(u - .5), fabs(v - .5)); // PLUS!
          if (cir < 0) z = VAR(_z1);
          else z = VAR(_z2);
        }
        break;
      case 4: // Original Modded (added VAR(width) for effect) 8 tiles
        square =  MAX(fabs(u - .5), fabs(v - .5)) - 0.5 * (1.0 - VAR(width));
        if (chance < 0.125) {
          if (((square < 0) ^ (u > v))) z = VAR(_z1);
          else z = VAR(_z2);
          ...
          if (((square < 0) ^ (u < 0.5))) z = VAR(_z1);
          else z = VAR(_z2);
        } else if (chance < 0.75) {
          if (((square < 0) ^ (v < 0.5))) z = VAR(_z1);
          else z = VAR(_z2);
        ...
        break;
      case 5: // circle filled truchet + VAR(width) for effect
        inv = (m % 2 == 0) ^ (n % 2 == 0);
        if (chance < 0.5) {
          cir = sqrt((u - 1.0) * (u - 1.0) + (v - 1.0) * (v - 1.0));
          cir = MIN(cir, sqrt(u * u + v * v));
          square = cir;
          cir = fabs(cir) - 0.5;
          // dots effect (suggested a VAR(width) = 0.25)
          square = MIN(square, sqrt((u - 1.0) * (u - 1.0) + v * v));
          square = MIN(square, sqrt(u * u + (v - 1.0) * (v - 1.0)));
          square = fabs(square) - VAR(width);
          if ((cir < 0) ^ (square < 0) ^ inv) z = VAR(_z1);
          else z = VAR(_z2);
        } else {
          cir = sqrt((u - 1.0) * (u - 1.0) + v * v);
          cir = MIN(cir, sqrt(u * u + (v - 1.0) * (v - 1.0)));
          ...
          cir = fabs(cir) - 0.5;
          // dots effect (suggested a VAR(width) = 0.25)
          square = MIN(square, sqrt((u - 1.0) * (u - 1.0) + (v - 1.0) * (v - 1.0)));
          square = MIN(square, sqrt(u * u + v * v));
          square = fabs(square) - VAR(width);
          if ((cir < 0) ^ (square < 0) ^ inv) z = VAR(_z2);
          else z = VAR(_z1);
        }
        break;
    }
