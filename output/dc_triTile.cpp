...
/*
    Apophysis Plugin: dc_triTile

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/DCTriTileFunc.java
...
typedef struct
{
	int type;
	int t;
	double col1; // Math.round(random01() * 35 + 10) / 100.0
	double col2; // Math.round(random01() * 35 + 55) / 100.0
	int on; // (int) (random01() * 2)
	int on1; // (int) (random01() * 2)
	int on2; // (int) (random01() * 2)
	int itt; // (int) (random01() * 5 + 1)
	int itt1; // (int) (random01() * 5)
	int itt2; // (int) (random01() * 5)
	int eq; // (int) (random01() * 2 + 1)
	int eq1; // (int) (random01() * 2 + 1)
	int eq2; // (int) (random01() * 2 + 1)
	int depth; // (int) (random01() * 4)
	int depth1; // (int) (random01() * 4)
...
double discretNoise (Variation* vp, int x) {
...
void fiveFoldInit (Variation* vp, int t, triangle orig) {
    ...
    VAR(_alfa) = 2 * cos(M_2PI / 5);
}

void fiveFoldDevis (Variation* vp, triangle in, triangle out) {

    // http://tilings.math.uni-bielefeld.de/substitution/penrose-triangle-without-rotations/
    // (original FiveFold VAR(type))
    double x4, y4, x5, y5;
    double w;

    switch (in.type) {
      case 1:
        x4 = in.x2 + VAR(_alfa) * (in.x3 - in.x2);
        y4 = in.y2 + VAR(_alfa) * (in.y3 - in.y2);

        w = GOODRAND_01();

        if (w < VAR(_alfa) * VAR(_alfa)) {
          out.type = 1;
          out.col = VAR(col1);
          out.x1 = in.x3;
          out.y1 = in.y3;
        ...
        break;
      case 2:
        x4 = in.x3 + VAR(_alfa) * (in.x1 - in.x3);
        y4 = in.y3 + VAR(_alfa) * (in.y1 - in.y3);
        x5 = in.x3 + VAR(_alfa) * (in.x2 - in.x3);
        y5 = in.y3 + VAR(_alfa) * (in.y2 - in.y3);
        ...
        if (w < VAR(_alfa) * VAR(_alfa)) {
          ...
          out.x3 = in.x1;
          out.y3 = in.y1;
        } else if (w < 2 * VAR(_alfa) * VAR(_alfa)) {
          out.type = 2;
          out.col = VAR(col2);
          out.x1 = in.x3;
          out.y1 = in.y3;
...
void trihexInit (Variation* vp, int t, triangle orig) {
...
void trihexDevis (Variation* vp, triangle in, triangle out) {
    ...
    if (w < 0.25) {
      out.type = in.type;
      out.col = in.type == 1 ? VAR(col1) : VAR(col2);
      out.x1 = x4;
      out.y1 = y4;
      out.x2 = x5;
      out.y2 = y5;
      ...
      out.type = 3 - in.type;
      out.col = in.type == 1 ? VAR(col2) : VAR(col1);
      out.x1 = x4;
      out.y1 = y4;
      out.x2 = x5;
      out.y2 = y5;
...
void equiThirdsInit (Variation* vp, int t, triangle orig) {
...
void equiThirdsDevis (Variation* vp, triangle in, triangle out) {
    ...
    switch (in.type) {
      case 1:
        ...
        x5 = x4 + VAR(_alfa) * (in.x2 - x4);
        y5 = y4 + VAR(_alfa) * (in.y2 - y4);
        ...
        break;
      case 2:
        x4 = in.x1 + VAR(_alfa) * (in.x3 - in.x1);
        y4 = in.y1 + VAR(_alfa) * (in.y3 - in.y1);
        x5 = in.x3 + VAR(_alfa) * (in.x1 - in.x3);
        y5 = in.y3 + VAR(_alfa) * (in.y1 - in.y3);
...
void ortInit (Variation* vp, int t, triangle orig) {
...
void ortDevis (Variation* vp, triangle in, triangle out) {

    // http://tilings.math.uni-bielefeld.de/substitution/overlapping-robinson-triangle-1/
    double x4, y4, x5, y5, x6, y6;
    double w;

    x4 = in.x2 + VAR(_alfa) * (in.x1 - in.x2);
    y4 = in.y2 + VAR(_alfa) * (in.y1 - in.y2);
    x5 = in.x1 + VAR(_alfa) * (in.x2 - in.x1);
    y5 = in.y1 + VAR(_alfa) * (in.y2 - in.y1);
    x6 = in.x2 + VAR(_alfa) * (in.x3 - in.x2);
    y6 = in.y2 + VAR(_alfa) * (in.y3 - in.y2);
...
void pinwheelInit (Variation* vp, int t, triangle orig) {
...
void pinwheelDevis (Variation* vp, triangle in, triangle out) {
...
void goldInit (Variation* vp, int t, triangle orig) {
    ...
    VAR(_alfa) = sqr(M_1_PHI);
}

void goldDevis (Variation* vp, triangle in, triangle out) {
    ...
    switch (in.type) {
      case 1:
        x4 = in.x1 + VAR(_alfa) * (in.x2 - in.x1);
        y4 = in.y1 + VAR(_alfa) * (in.y2 - in.y1);
        x5 = in.x2 + VAR(_alfa) * (in.x1 - in.x2);
        y5 = in.y2 + VAR(_alfa) * (in.y1 - in.y2);
        x6 = in.x3 + VAR(_alfa) * (in.x2 - in.x3);
        y6 = in.y3 + VAR(_alfa) * (in.y2 - in.y3);
        x7 = in.x1 + VAR(_alfa) * (in.x3 - in.x1);
        y7 = in.y1 + VAR(_alfa) * (in.y3 - in.y1);
      ...
      case 2:
        x4 = in.x2 + VAR(_alfa) * (in.x1 - in.x2);
        y4 = in.y2 + VAR(_alfa) * (in.y1 - in.y2);
        x5 = in.x2 + VAR(_alfa) * (in.x3 - in.x2);
        y5 = in.y2 + VAR(_alfa) * (in.y3 - in.y2);
        x6 = in.x3 + VAR(_alfa) * (in.x2 - in.x3);
        y6 = in.y3 + VAR(_alfa) * (in.y2 - in.y3);
        x7 = in.x1 + VAR(_alfa) * (in.x3 - in.x1);
        y7 = in.y1 + VAR(_alfa) * (in.y3 - in.y1);
        x8 = in.x3 + VAR(_alfa) * (in.x1 - in.x3);
        y8 = in.y3 + VAR(_alfa) * (in.y1 - in.y3);
        x9 = x4 + VAR(_alfa) * (x7 - x4);
        y9 = y4 + VAR(_alfa) * (y7 - y4);
        x10 = x7 + VAR(_alfa) * (x4 - x7);
        y10 = y7 + VAR(_alfa) * (y4 - y7);
...
void devis (Variation* vp, triangle t1, triangle t2) {

    switch (VAR(type)) {
      case 1:
        fiveFoldDevis(vp, t1, t2);
        break;
      case 2:
        trihexDevis(vp, t1, t2);
        break;
      case 3:
        equiThirdsDevis(vp, t1, t2);
        break;
      case 4:
        ortDevis(vp, t1, t2);
        break;
      case 5:
        pinwheelDevis(vp, t1, t2);
        break;
      case 6:
        goldDevis(vp, t1, t2);
        break;
...
int PluginVarPrepare(Variation* vp)
{
    ...
    switch (VAR(type)) {
      case 1:
        fiveFoldInit(vp, VAR(t), orig);
        break;
      case 2:
        trihexInit(vp, VAR(t), orig);
        break;
      case 3:
        equiThirdsInit(vp, VAR(t), orig);
        break;
      case 4:
        ortInit(vp, VAR(t), orig);
        break;
      case 5:
        pinwheelInit(vp, VAR(t), orig);
        break;
      case 6:
        goldInit(vp, VAR(t), orig);
        break;
    }
...
int PluginVarCalc(Variation* vp)
{
    ...
    while (n > 0) {
      ...
      if (VAR(on) == 0) {
        if ((k == VAR(itt)) && (t1.type != VAR(eq))) n = n + VAR(depth);
      } else {
        if ((k >= VAR(itt)) && (k < VAR(itt) + VAR(depth)) && (t1.type != VAR(eq))) n = n + 1;
      }
      n = n - 1;
      k = k + 1;
    }
    ...
    switch (t1.type) {
      case 1:
        ...
        while (n > 0) {
          ...
          m = 2 * m + t2.type - 1;
          if (VAR(on1) == 0) {
            if ((k == VAR(itt1)) && (t1.type != VAR(eq1))) n = n + VAR(depth1);
          } else {
            if ((k >= VAR(itt1)) && (k < VAR(itt1) + VAR(depth1)) && (t1.type != VAR(eq1))) n = n + 1;
          }
          n = n - 1;
          k = k + 1;
        }
        t1.col = discretNoise(vp, m + 43);
        break;
      case 2:
        n = VAR(itt2);
        k = 1;
        while (n > 0) {
          ...
          m = 2 * m + t2.type - 1;
          if (VAR(on2) == 0) {
            if ((k == VAR(itt2)) && (t1.type != VAR(eq2))) n = n + VAR(depth2);
          } else {
            if ((k >= VAR(itt2)) && (k < VAR(itt2) + VAR(depth2)) && (t1.type != VAR(eq2))) n = n + 1;
          }
          n = n - 1;
          k = k + 1;
        }
        t1.col = discretNoise(vp, m + 31);
        break;
    }
