...
{
    VAR(_bCn_pi) = (double) VAR(num) * M_1_PI;
    VAR(_pi_bCn) = M_PI / (double) VAR(num);
    VAR(_bCa) = M_PI * VAR(a);
    VAR(_bCa_bCn) = VAR(_bCa) / (double) VAR(num);
...
int PluginVarCalc(Variation* vp)
{
    ...
    alt = (int) (sigma * VAR(_bCn_pi));
    if (alt % 2 == 0)
      sigma = alt * VAR(_pi_bCn) + fmod(sigma + VAR(_bCa_bCn), VAR(_pi_bCn));
    else
      sigma = alt * VAR(_pi_bCn) + fmod(sigma - VAR(_bCa_bCn), VAR(_pi_bCn));
    sinht = sinh(tau);
    cosht = cosh(tau);
