...
/*
    Apophysis Plugin: triantruchet

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/TrianTruchetFunc.java
...
int PluginVarPrepare(Variation* vp)
{
    ...
    tileSize = (float) VAR(Size) / VAR(TilesPerRow);
    VAR(_numberTiles) = VAR(TilesPerRow) * VAR(TilesPerColumn);
    GOODRAND_SEED(VAR(seed));
    GOODRAND_01();
    VAR(_n1) = (int) (GOODRAND_01() * 4);
    VAR(_n2) = (int) (GOODRAND_01() * 4);
    VAR(_n3) = (int) (GOODRAND_01() * 4);
    VAR(_n4) = (int) (GOODRAND_01() * 4);
...
int PluginVarCalc(Variation* vp)
{
    ...
    if (VAR(seed) == 0) {
      try {
        VAR(_n1) = Integer.parseInt(string.substring(0, 1));
        VAR(_n2) = Integer.parseInt(string.substring(1, 2));
        VAR(_n3) = Integer.parseInt(string.substring(2, 3));
        VAR(_n4) = Integer.parseInt(string.substring(3, 4));
      } catch (Exception e) {
      }
    }
    if (i % 2 == 0) // i odd (par)
    ...
    double color = (VAR(TilesPerRow) * j + i) / (double) VAR(_numberTiles);

    Point dot = plotTriangle(vp, t);

    FPx += VVAR * (dot.getX() - tileSize * VAR(TilesPerRow) / 2.0);
    FPy += VVAR * (dot.getY() - tileSize * VAR(TilesPerColumn) / 2.0);
    TC = color;
