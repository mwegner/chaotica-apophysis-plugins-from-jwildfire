...
/*
    Apophysis Plugin: waffle

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/WaffleFunc.java
...
{
    VAR(_vcosr) = VVAR * cos(VAR(rotation));
    VAR(_vsinr) = VVAR * sin(VAR(rotation));
...
int PluginVarCalc(Variation* vp)
{
    ...
    double a = 0.0, r = 0.0;
    switch (GOODRAND_0X(5)) {
      case 0:
        a = (GOODRAND_0X(VAR(slices)) + GOODRAND_01() * VAR(xthickness)) / VAR(slices);
        r = (GOODRAND_0X(VAR(slices)) + GOODRAND_01() * VAR(ythickness)) / VAR(slices);
        break;
      case 1:
        a = (GOODRAND_0X(VAR(slices)) + GOODRAND_01()) / VAR(slices);
        r = (GOODRAND_0X(VAR(slices)) + VAR(ythickness)) / VAR(slices);
        break;
      case 2:
        a = (GOODRAND_0X(VAR(slices)) + VAR(xthickness)) / VAR(slices);
        r = (GOODRAND_0X(VAR(slices)) + GOODRAND_01()) / VAR(slices);
        break;
      case 3:
        a = GOODRAND_01();
        r = (GOODRAND_0X(VAR(slices)) + VAR(ythickness) + GOODRAND_01() * (1 - VAR(ythickness))) / VAR(slices);
        break;
      case 4:
        a = (GOODRAND_0X(VAR(slices)) + VAR(xthickness) + GOODRAND_01() * (1 - VAR(xthickness))) / VAR(slices);
        r = GOODRAND_01();
        break;
      default: // nothing to do
        break;
    }
    FPx += (VAR(_vcosr) * a + VAR(_vsinr) * r); // note that post-transforms make this redundant!
    FPy += (-VAR(_vsinr) * a + VAR(_vcosr) * r);
