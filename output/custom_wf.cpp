...
/*
    Apophysis Plugin: custom_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/CustomWFFunc.java
...
void compile (Variation* vp) {
    ...
    try {
      customFuncRunner = CustomWFFuncRunner.compile(vp, code);
    } catch (Throwable ex) {
      System.out.println("##############################################################");
...
void validate (Variation* vp) {
    ...
    try {
      if (code != null) {
        CustomWFFuncRunner.compile(vp, code);
      }
    } catch (Throwable ex) {
...
int PluginVarPrepare(Variation* vp)
{
    ...
    if (customFuncRunner != null) {
      customFuncRunner.setA(VAR(a));
      customFuncRunner.setB(VAR(b));
      customFuncRunner.setC(VAR(c));
      customFuncRunner.setD(VAR(d));
      customFuncRunner.setE(VAR(e));
      customFuncRunner.setF(VAR(f));
      customFuncRunner.setG(VAR(g));
      customFuncRunner.init(vp, pXForm);
    }
