...
/*
    Apophysis Plugin: glynnia3

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Glynnia3Func.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    double r = VAR(rscale) * (sqrt(sqr(FTx) + sqr(FTy)));
    double d;

    if (r > VAR(rthresh) && FTy > VAR(ythresh)) {
      if (GOODRAND_01() > 0.5) {
        d = VAR(dscale) * sqrt(r + FTx);
        if (d == 0) {
          return TRUE;
        }
        FPx += VAR(_vvar2) * d;
