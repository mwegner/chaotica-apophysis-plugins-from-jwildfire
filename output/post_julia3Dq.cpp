...
/*
    Apophysis Plugin: post_julia3Dq

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/PostJulia3DQFunc.java
...
#define APO_VARIABLE_PREFIX "post_julia3Dq_"
#include "plugin.h"
...
{
    VAR(_inv_power) = (double) VAR(divisor) / (double) VAR(power);
    VAR(_abs_inv_power) = fabs(VAR(_inv_power));
    VAR(_half_inv_power) = 0.5 * VAR(_inv_power) - 0.5;
...
int PluginVarCalc(Variation* vp)
{
    ...
    double sina = sin(a);
    double cosa = cos(a);
    double z = FPz * VAR(_abs_inv_power);
    double r2d = sqr(FPx) + sqr(FPy);
    double r = VVAR * pow(r2d + sqr(z), VAR(_half_inv_power));
    FPz = r * z;
    r *= sqrt(r2d);
    FPx = r * cosa;
    FPy = r * sina;
