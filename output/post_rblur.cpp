...
int PluginVarCalc(Variation* vp)
{
    ...
    double r = sqrt(sqr(FPx - VAR(center_x)) + sqr(FPy - VAR(center_y))) - VAR(offset);
    r = r < 0 ? 0 : r;
    r *= VAR(_s2);
    FPx = VVAR * (FPx + (GOODRAND_01() - 0.5) * r);
    FPy = VVAR * (FPy + (GOODRAND_01() - 0.5) * r);
