...
/*
    Apophysis Plugin: bipolar

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/BipolarFunc.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    double x2y2 = (FTx * FTx + FTy * FTy);
    double t = x2y2 + 1.0;
    double x2 = 2 * FTx;
    double ps = -M_PI_2 * VAR(shift);
    double y = 0.5 * atan2(2.0 * FTy, x2y2 - 1.0) + ps;
    ...
    if ((g == 0) || (f / g <= 0))
      return TRUE;
    FPx += VVAR * 0.25 * M_2_PI * log((t + x2) / (t - x2));
    FPy += VVAR * M_2_PI * y;
