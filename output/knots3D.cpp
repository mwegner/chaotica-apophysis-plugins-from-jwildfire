...
/*
    Apophysis Plugin: knots3D

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Knots3DFunc.java
...
APO_PLUGIN("knots3D");
APO_VARIABLES(
	VAR_INTEGER(presetId, -1),
	VAR_INTEGER(steps, 1500),
	VAR_REAL(radius, 2.0),
	VAR_INTEGER(facets, 4),
	VAR_REAL(param_a, 1.0),
	VAR_REAL(param_b, 1.0),
	VAR_REAL(param_c, 1.0),
	VAR_REAL(param_d, 1.0),
...
void validatePresetId (Variation* vp) {

    if (VAR(presetId) >= 0) {
      Knots3DWFFuncPreset preset = WFFuncPresetsStore.getKnots3DWFFuncPresets().getPreset(VAR(presetId));
...
void refreshFormulaFromPreset (Variation* vp, int presetId) {
    ...
    VAR(steps) = (int) preset.getSteps();
    VAR(radius) = preset.getRadius();
    VAR(facets) = (int) preset.getFacets();

    VAR(param_a) = preset.getParam_a();
    VAR(param_b) = preset.getParam_b();
    VAR(param_c) = preset.getParam_c();
    VAR(param_d) = preset.getParam_d();
    VAR(param_e) = preset.getParam_e();
    VAR(param_f) = preset.getParam_f();
    VAR(param_g) = preset.getParam_g();
    VAR(param_h) = preset.getParam_h();
...
boolean dynamicParameterExpansion (Variation* vp, String pName) {
    ...
    // VAR(presetId) doesn't really expand parameters, but it changes them; this will make them refresh
