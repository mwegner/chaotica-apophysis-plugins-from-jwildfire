...
APO_VARIABLES(
    VAR_REAL_RANGE(jac_dn_k, -1.0, 1.0, 0.5)
);
...
int PluginVarCalc(Variation* vp)
{
    ...
    Jacobi_elliptic(FTx,   VAR(jac_dn_k)   ,&snx,&cnx,&dnx);
    Jacobi_elliptic(FTy, 1.0-VAR(jac_dn_k) ,&sny,&cny,&dny);
    
    NumX = dnx*cny*dny;
    NumY = -cnx*snx*sny*VAR(jac_dn_k);
    Denom = sqr(snx)*sqr(sny)*VAR(jac_dn_k) + sqr(cny);
