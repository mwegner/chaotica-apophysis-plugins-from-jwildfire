...
/*
    Apophysis Plugin: ducks

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/DucksFunc.java
...
typedef struct
{
	double _escape = 5.0; //0.0-11.0
	double _C = 1.0;       //0.0-2.0
	double _colorFactor = 1.0;  //0.0-1.0
	int _size = 1500;
	int _zoomIn = 1;
	int _zoomOut = 1;
	double _c_re = 0.672;
	double _c_im = 0.0; // -0.378
	double _colorScale = 1.0;
	double _colorCycle = 2.0;
	double _colorOffset = 0.0;
	int _colortype = 4;
	int _colorMirror = 1;
	int _colorRainbow;
	int _juliamode;
...
void compile (Variation* vp) {
    ...
    try {
      cf_runner = ComplexFuncRunner.compile(vp, code_func);
    } catch (Throwable ex) {
      System.out.println("##############################################################");
...
void validate (Variation* vp) {
    ...
    try {
      if (code_func != null) {
        ComplexFuncRunner.compile(vp, code_func);
      }
    } catch (Throwable ex) {
...
double clamp (Variation* vp, double x, double minVal, double maxVal) {

    return MIN(MAX(x, minVal), maxVal);
}

double mix (Variation* vp, double x, double y, double a) {
