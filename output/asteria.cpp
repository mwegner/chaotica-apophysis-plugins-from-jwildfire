...
/*
    Apophysis Plugin: asteria

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/AsteriaFunc.java
...
#define APO_VARIABLE_PREFIX "asteria_"
#include "plugin.h"
...
{
    VAR(_sina) = sin(M_PI * VAR(alpha));
    VAR(_cosa) = cos(M_PI * VAR(alpha));
...
int PluginVarCalc(Variation* vp)
{
    ...
    if (in1) { // linear
      ...
      xx = x0 * VAR(_cosa) - y0 * VAR(_sina);
      yy = x0 * VAR(_sina) + y0 * VAR(_cosa);
      double nx = xx / sqrt(1.0 - yy * yy) * (1.0 - sqrt(1.0 - sqr(-fabs(yy) + 1.0)));
      xx = nx * VAR(_cosa) + yy * VAR(_sina);
      yy = -nx * VAR(_sina) + yy * VAR(_cosa);
      FPx += xx;
      FPy += yy;
      if (true /* pContext\.isPreserveZCoordinate() */) {
        FPz += VVAR * FTz;
