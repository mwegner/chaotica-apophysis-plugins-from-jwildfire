...
/*
    Apophysis Plugin: polylogarithm

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/PolylogarithmFunc.java
...
double HarmonicS (Variation* vp, int N) {
 // Defined in Crandall's paper
    if (VAR(_N) < 1)
      return 0.0;
    double Hs = 0.0;
    int i = 1;
...
int PluginVarPrepare(Variation* vp)
{
    VAR(_N) = VAR(n);
    Riem = new RiemannInt();
    VAR(_HSTerm) = HarmonicS(vp, (int) VAR(_N) - 1);
...
int PluginVarCalc(Variation* vp)
{
    ...
    // approx (very good) of Li[VAR(n)](z) for VAR(n) > 1
    double vv = VVAR;
    Complex z = new Complex(FTx, FTy);
    z.Pow(VAR(zpow));
    z.Save();
    if (z.Mag2() > 250000.0 || VAR(_N) >= 20) { // no convergence, or VAR(_N) too big... When VAR(_N) is big then Li tends to z
      FPx += vv * z.re;
      FPy += vv * z.im;
      return TRUE;
    }
    ...
    if (z.Mag2() < 0.07) { // normal series. Li = sum((z^k)/(k^VAR(n)))
      for (i = 1; i < 20; i++) {
        T.Copy(new Complex(pow(i, VAR(_N))));
        T.DivR(z);
        LiN.Add(T);
        z.NextPow();
      }
    ...
    for (i = 0; i < 20; i++) {
      double zee = Riem.Z((int) VAR(_N) - i);
      if (zee != 0.0) {
        T.Copy(z);
    ...
    T.Copy(z);
    z.Copy(zPl1);
    z.Scale(1.0 / cern.jet.math.Arithmetic.longFactorial((int) VAR(_N) - 1));
    z.Mul(T);
    LiN.Add(z);
    FPx += vv * LiN.re;
    FPy += vv * LiN.im;
