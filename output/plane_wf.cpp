...
/*
    Apophysis Plugin: plane_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/PlaneWFFunc.java
...
void setColor (Variation* vp, XYZPoint pVarTP, double u, double v) {

    if (VAR(direct_color) > 0) {
      switch (VAR(color_mode)) {
        case CM_V:
          TC = v + 0.5;
          break;
        case CM_UV:
        ...
        case CM_COLORMAP: {
          ...
          if (VAR(calc_color_idx) == 1) {
            TC = getUVColorIdx(vp, (pVarTP.redColor), (pVarTP.greenColor), (pVarTP.blueColor));
          }
        }
        break;
        case CM_U:
...
double getUVColorIdx (Variation* vp, int pR, int pG, int pB) {
...
int PluginVarCalc(Variation* vp)
{
    ...
    switch (VAR(axis)) {
      case AXIS_XY:
        x = 0.5 - GOODRAND_01();
        y = 0.5 - GOODRAND_01();
        z = VAR(position) + getDisplacement(vp, x, y);
        setColor(vp, pVarTP, x, y);
        x *= VAR(size);
        y *= VAR(size);
        break;
      case AXIS_YZ:
        y = 0.5 - GOODRAND_01();
        z = 0.5 - GOODRAND_01();
        x = VAR(position) + getDisplacement(vp, y, z);
        setColor(vp, pVarTP, y, z);
        y *= VAR(size);
        z *= VAR(size);
        break;
      case AXIS_ZX:
        x = 0.5 - GOODRAND_01();
        z = 0.5 - GOODRAND_01();
        y = VAR(position) + getDisplacement(vp, z, x);
        setColor(vp, pVarTP, z, x);
        x *= VAR(size);
        z *= VAR(size);
        break;
