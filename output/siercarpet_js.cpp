...
int PluginVarCalc(Variation* vp)
{
    ...
    for (int i = 1; i <= VAR(_m); i++) {
      VAR(_a)[2 * i - 1] = cos(2.0 * M_PI * (double) i / (double) VAR(_m));
      VAR(_b)[2 * i - 1] = sin(2.0 * M_PI * (double) i / (double) VAR(_m));
    }
    for (int i = 1; i <= VAR(_m); i++) {
      VAR(_a)[2 * i] = (cos(2.0 * M_PI * i / (double) VAR(_m)) + cos(2.0 * M_PI * (i - 1) / (double) VAR(_m))) / 2.0;
      VAR(_b)[2 * i] = (sin(2.0 * M_PI * i / (double) VAR(_m)) + sin(2.0 * M_PI * (i - 1) / (double) VAR(_m))) / 2.0;
    }

    int l = 1 + (int) (2.0 * (double) VAR(_m) * GOODRAND_01());

    if (VAR(_d) % (2 * VAR(_m)) != (5) % 2 * VAR(_m)) {
      VAR(_d) = 1;
      x = FTx / 3.0 + (VAR(_a)[l] - VAR(_b)[l]) / sqrt(2.0);
      y = FTy / 3.0 + (VAR(_a)[l] + VAR(_b)[l]) / sqrt(2.0);
    }
