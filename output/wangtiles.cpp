...
/*
    Apophysis Plugin: wangtiles

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/WangTilesFunc.java
...
APO_PLUGIN("wangtiles");
APO_VARIABLES(
	VAR_INTEGER(id, 0),
	VAR_INTEGER(seed, 1000),
	VAR_INTEGER(square, 1),
	VAR_REAL(scale_x, 1.0),
...
void transformImage (Variation* vp, XForm pXForm double pAmount, double pInputX, double pInputY) {

    double x = (pInputX - (VAR(offset_x) + 0.5) + 1.0) / VAR(scale_x) * (double) (VAR(_imgWidth) - 2);
    double y = (pInputY - (VAR(offset_y) + 0.5) + 1.0) / VAR(scale_y) * (double) (VAR(_imgHeight) - 2);
    int ix, iy;
    if (VAR(_blend_colormap) > 0) {
    ...
    if (VAR(tile_x) == 1) {
      if (ix < 0) {
        int nx = ix / VAR(_imgWidth) - 1;
        ix -= nx * VAR(_imgWidth);
      } else if (ix >= VAR(_imgWidth)) {
        int nx = ix / VAR(_imgWidth);
        ix -= nx * VAR(_imgWidth);
      }
    }
    if (VAR(tile_y) == 1) {
      if (iy < 0) {
        int ny = iy / VAR(_imgHeight) - 1;
        iy -= ny * VAR(_imgHeight);
      } else if (iy >= VAR(_imgHeight)) {
        int ny = iy / VAR(_imgHeight);
        iy -= ny * VAR(_imgHeight);
      }
    }

    double r, g, b;
    if (ix >= 0 && ix < VAR(_imgWidth) && iy >= 0 && iy < VAR(_imgHeight)) {
    ...
    double dz = VAR(offset_z);
    if (fabs(VAR(scale_z)) > EPSILON) {
      double intensity = (0.299 * r + 0.588 * g + 0.113 * b) / 255.0;
      dz += VAR(scale_z) * intensity;
    }
    if (VAR(reset_z) != 0) {
      FPz = dz;
    } else {
    ...
    if (VAR(_dc_color) > 0) {
      TC = getColorIdx(vp, (r), (g), (b));
    }
}

double getColorIdx (Variation* vp, int pR, int pG, int pB) {
...
void clearCurrColorMap (Variation* vp) {
...
void initOnce (Variation* vp, Layer pLayer, XForm pXForm, double pAmount) {
    ...
    try {
      ...
      if (colorMap == null) {
        ...
        String option = new String(Id[VAR(id)]);
...
int PluginVarPrepare(Variation* vp)
{
   VAR(_dc_color) = 1;
    ...
    VAR(_imgWidth) = colorMap.getImageWidth();
    VAR(_imgHeight) = colorMap.getImageHeight();
...
int PluginVarCalc(Variation* vp)
{
    ...
    FPx += VVAR * rect * (GOODRAND_01() - 0.5);
    FPy += VVAR * (GOODRAND_01() - 0.5);

    transformImage(vp, pXForm, pAffineTP, pVarTP, VVAR, FPx, FPy);
