...
/*
    Apophysis Plugin: szubieta

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/SZubietaFunc.java
...
void build_pattern2 (Variation* vp) {
  // Square Tile Fractal pattern
    ...
    for (int i = 0; i < VAR(width); ++i) {
      for (int j = 0; j < VAR(height); ++j) {
        ...
        double x1 = (double) i - VAR(width) / 2.0;
        double y1 = (double) j - VAR(height) / 2.0;
        primitives.add(new Ngon(20, VAR(scale), 0.0, new Point(x1, y1), color, 0.0));
      }
    }
}

void build_pattern1 (Variation* vp) {
 // CircleSquares pattern
...
int PluginVarCalc(Variation* vp)
{
    ...
    if (primitive.gettype() == 4) {
      ...
      FPx += VVAR * 0.1 * (out.getX() * polygon.getScale() * polygon.getCosa() + out.getY() * polygon.getScale() * polygon.getSina() + polygon.getPos().getX());
      FPy += VVAR * 0.1 * (-out.getX() * polygon.getScale() * polygon.getSina() + out.getY() * polygon.getScale() * polygon.getCosa() + polygon.getPos().getY());
      TC = color;
