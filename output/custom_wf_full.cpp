...
/*
    Apophysis Plugin: custom_wf_full

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/CustomFullVariationWrapperFunc.java
...
#define APO_VARIABLE_PREFIX "custom_wf_full_"
#include "plugin.h"

APO_PLUGIN("custom_wf_full");
APO_VARIABLES(
...
void invtransform (Variation* vp) {
    ...
    if (full_variation != null) {
      full_variation.invtransform(vp, vp, pContext);
    }
}

int getParameterIndex (Variation* vp, String pName) {

    if (full_variation != null) {
      return full_variation.getParameterIndex(vp, pName);
    } else {
      return -1;
    }
}
...
void setRessource (Variation* vp, String pName, byte[] pValue) {
    ...
    if (RESSOURCE_CODE.equalsIgnoreCase(pName)) {
      if (pValue == null) {
        ...
        validate(vp); // compiles
      }
    } else if (full_variation != null) {
      full_variation.setRessource(vp, pName, pValue);
    } else {
...
void compile (Variation* vp) {
    ...
    if (DEBUG) {
      System.out.println("called compile(vp)");
    }
          ...
          for (String prev_param : prev_params) {
            ...
            if (prev_val != null && cur_val != null) {
              if (prev_val instanceof Number) {
                full_variation.setParameter(vp, prev_param, ((Number) prev_val).doubleValue());
                if (DEBUG) {
...
boolean ressourceCanModifyParams (Variation* vp, String resourceName) {

    if (resourceName.equalsIgnoreCase(RESSOURCE_CODE)) {
      return true;
    } else {
      return full_variation.ressourceCanModifyParams(vp, vp, resourceName);
    }
}

boolean ressourceCanModifyParams (Variation* vp) {
...
boolean dynamicParameterExpansion (Variation* vp, String paramName) {
    ...
    return full_variation.dynamicParameterExpansion(vp, vp, paramName);
}

boolean dynamicParameterExpansion (Variation* vp) {

    return full_variation != null ? full_variation.dynamicParameterExpansion(vp, vp, ) : false;
}
...
int PluginVarPrepare(Variation* vp)
{
    ...
    if (full_variation != null) {
      full_variation.initOnce(vp, pLayer, pXForm, VVAR);
      full_variation.init(vp, pLayer, pXForm, VVAR);
    }
