...
/*
    Apophysis Plugin: sineblur

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/SineBlurFunc.java
...
#define APO_VARIABLE_PREFIX "sineblur_"
#include "plugin.h"
...
int PluginVarCalc(Variation* vp)
{
    ...
    double VAR(power) = VAR(power);
    if (VAR(power) < 0.0)
      VAR(power) = 0.0;
    double ang = GOODRAND_01() * M_2PI;
    double r = VVAR * (VAR(power) == 1.0 ? acos(GOODRAND_01() * 2.0 - 1.0) / M_PI : acos(exp(log(GOODRAND_01()) * VAR(power)) * 2.0 - 1.0) / M_PI);
    double s = sin(ang);
    double c = cos(ang);
    FPx += r * c;
    FPy += r * s;
