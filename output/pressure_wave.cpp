...
int PluginVarCalc(Variation* vp)
{
    ...
    // multiplications and stuff moved into setParameter for speedup plus avoid 1.0/0.

    FPx += VVAR * (FTx + (VAR(_ipwx) * sin(VAR(_pwx) * FTx)));
    FPy += VVAR * (FTy + (VAR(_ipwy) * sin(VAR(_pwy) * FTy)));
