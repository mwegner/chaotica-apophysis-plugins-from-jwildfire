...
/*
    Apophysis Plugin: sunvoroni

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/SunflowerVoroniFunc.java
...
APO_PLUGIN("sunvoroni");
APO_VARIABLES(
	VAR_INTEGER(nPoints, 50),
	VAR_INTEGER(Iters, 3),
...
void build_with_mesh (Variation* vp) {


    double ang = VAR(angle) * (3.0 - sqrt(5.0));
    double rmax = sqrt((double) (VAR(nPoints) + 1)) / 30.0;
    points = new float[VAR(nPoints)][2];
    for (int i = 0; i < VAR(nPoints); i++) {
      double r = sqrt((double) (i + 1)) / 30.0;
      double t = (double) (i + 1) * ang * M_PI / 180.0;
      double x = r * cos(t) / rmax;
      double y = r * sin(t) / rmax;
      points[i][0] = (float) x;
      points[i][1] = (float) y;
    ...
    if (VAR(outline) == 1) {
      for (int i = 0; i < myEdges.length; i++) {
        ...
        primitives.add(new Line(startX, startY, endX, endY, VAR(_outlinecolor)));
      }
    }
    ...
    if (VAR(fill) == 1) {
      ...
      for (int i = 0; i < myRegions.length; i++) {
        ...
        Random GOODRAND_SEED(area);

        if (VAR(_colormode) == 0)
          color = fmod(1.0 + area, 1.0);
        else if (VAR(_colormode) == 1)
        ...
        for (int j = 0; j < tcount; j++) {
          Point p1 = result.get(j * 3 + 0);
          Point p2 = result.get(j * 3 + 1);
          Point p3 = result.get(j * 3 + 2);
          if (VAR(_colormode) == 2)
            color = GOODRAND_01();
...
{
   VAR(_outlinecolor) = 0.5;

    VAR(_line_thickness) = 0.5 / 100;
    if (VAR(outline) == 0 && VAR(fill) == 0)
      VAR(outline) = 1;
//   build_pattern();
    build_with_mesh(vp);
