...
APO_VARIABLES(
    VAR_REAL_RANGE(jac_cn_k, -1.0, 1.0, 0.5)
);
...
int PluginVarCalc(Variation* vp)
{
    ...
    Jacobi_elliptic(FTx,   VAR(jac_cn_k)   ,&snx,&cnx,&dnx);
    Jacobi_elliptic(FTy, 1.0-VAR(jac_cn_k) ,&sny,&cny,&dny);
    
    NumX = cnx*cny;
    NumY = -dnx*snx*dny*sny;
    Denom = sqr(snx)*sqr(sny)*VAR(jac_cn_k) + sqr(cny);
