...
/*
    Apophysis Plugin: htree_js

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/HtreeFunc.java
...
#define APO_VARIABLE_PREFIX "htree_js_"
#include "plugin.h"
...
void drawH (Variation* vp, double x, double y, double size) {


    // compute the coordinates of the 4 tips of the H
    double x0 = x - VAR(size) / 2;
    double x1 = x + VAR(size) / 2;
    double y0 = y - VAR(size) / 2;
    double y1 = y + VAR(size) / 2;
...
void draw (Variation* vp, int n, double x, double y, double size) {

    if (n == 0)
      return;
    drawH(vp, x, y, VAR(size));

    // compute x- and y-coordinates of the 4 half-VAR(size) H-trees
    double x0 = x - VAR(size) / 2;
    double x1 = x + VAR(size) / 2;
    double y0 = y - VAR(size) / 2;
    double y1 = y + VAR(size) / 2;

    // recursively draw 4 half-VAR(size) H-trees of order n-1
    draw(vp, n - 1, x0, y0, VAR(size) / 2); // lower left  H-tree
    draw(vp, n - 1, x0, y1, VAR(size) / 2); // upper left  H-tree
    draw(vp, n - 1, x1, y0, VAR(size) / 2); // lower right H-tree
    draw(vp, n - 1, x1, y1, VAR(size) / 2); // upper right H-tree
}
