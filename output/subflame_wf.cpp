...
/*
    Apophysis Plugin: subflame_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/SubFlameWFFunc.java
...
typedef struct
{
	double angle;
	double scale;
	double _cosa; // cos(angle * M_PI / 180.0)
	double _sina; // sin(angle * M_PI / 180.0)
	double offset_x;
	double offset_y;
	double offset_z;
	double colorscale_z;
	int _flame_is_sequence;
	int _flame_sequence_start = 1;
	int _flame_sequence_end;
	int _flame_sequence_repeat = 1;
	int _flame_sequence_digits = 4;
	int color_mode; // CM_OFF
	int _frame;

    int ___warning;
} Variables;

#define APO_VARIABLE_PREFIX "subflame_wf_"
#include "plugin.h"

APO_PLUGIN("subflame_wf");
APO_VARIABLES(
	VAR_REAL(angle, 0.0),
	VAR_REAL(scale, 1.0),
...
{
   VAR(_flame_sequence_start) = 1;
   VAR(_flame_sequence_repeat) = 1;
   VAR(_flame_sequence_digits) = 4;
    int seq_start = VAR(_flame_sequence_start) < 1 ? 1 : VAR(_flame_sequence_start);
    int seq_end = VAR(_flame_sequence_end) <= 0 ? 0 : VAR(_flame_sequence_end) < seq_start ? seq_start : VAR(_flame_sequence_end);
    int seq_repeat = VAR(_flame_sequence_repeat) < 1 || VAR(_flame_sequence_repeat) > seq_end ? seq_end : VAR(_flame_sequence_repeat);
    int seq_period = seq_end - seq_repeat + 1;
    VAR(_frame) = pContext.getFrame() + VAR(_flame_sequence_start) - 1;
    if (seq_end > 0 && VAR(_frame) > seq_end) VAR(_frame) = (VAR(_frame) - seq_repeat) % seq_period + seq_repeat;

    parseFlame(vp);
    int time = VAR(_flame_is_sequence) == SEQ_CURVE ? VAR(_frame) : flame.getFrame() > 0 ? flame.getFrame() : 0;
