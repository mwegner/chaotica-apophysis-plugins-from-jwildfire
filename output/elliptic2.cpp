...
/*
    Apophysis Plugin: elliptic2

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Elliptic2Func.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    double tmp = FTy * FTy + FTx * FTx + VAR(a1);
    double x2 = VAR(b1) * FTx;
    double xmax = VAR(c) * (sqrt(tmp + x2) + sqrt(tmp - x2));

    double a = FTx / xmax * VAR(a2);
    double b = sqrt_safe(vp, VAR(d) - a * a) * VAR(b2);
    double ps = -M_PI_2 * VAR(a3);
    FPx += VAR(_v) * atan2(a, b) + ps;

    //    if (FTy > 0)
    if (GOODRAND_01() < VAR(e))
      FPy += VAR(_v) * log(xmax + sqrt_safe(vp, xmax - VAR(f)));
    else
      FPy -= VAR(_v) * log(xmax + sqrt_safe(vp, xmax - VAR(g)));
