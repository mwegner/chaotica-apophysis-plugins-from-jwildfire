...
{
    VAR(_pc_xlen) = VAR(xlength) * VAR(xlength);
    VAR(_pc_ylen) = VAR(ylength) * VAR(ylength);
    if (VAR(_pc_xlen) < 1E-20)
      VAR(_pc_xlen) = 1E-20;
    if (VAR(_pc_ylen) < 1E-20)
      VAR(_pc_ylen) = 1E-20;
...
int PluginVarCalc(Variation* vp)
{
    ...
    FPx += VVAR * (FTx + VAR(xamp) * exp(-FTy * FTy / VAR(_pc_xlen)));
    FPy += VVAR * (FTy + VAR(yamp) * exp(-FTx * FTx / VAR(_pc_ylen)));
