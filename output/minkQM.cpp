...
/*
    Apophysis Plugin: minkQM

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/MinkQMFunc.java
...
double minkowski (Variation* vp, double x) {
    ...
    double q = VAR(a), r = p + VAR(b), s = VAR(c), m = 0.0, n = 0.0;
    double d = VAR(dd), y = p;
    for (int it = 0; it < VAR(f); it++) {
      d = d * VAR(e); //if (d<1e-10) break;
      m = p + r;
      n = q + s;
      if (x < (m / n)) {
        r = m;
