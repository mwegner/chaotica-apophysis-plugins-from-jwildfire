...
/*
    Apophysis Plugin: dragon_js

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/DragonFunc.java
...
#define APO_VARIABLE_PREFIX "dragon_js_"
#include "plugin.h"

APO_PLUGIN("dragon_js");
APO_VARIABLES(
	VAR_INTEGER(level, 2),
...
void nogard (Variation* vp, int n) {
...
void dragon (Variation* vp, int n) {
...
void Dragon (Variation* vp, int n) {
     ...
     *  to the left in dragon(vp, i).  right[i], up[i], down[i] are similar.
     ***********************************************************************/
    ...
    //     double size = MAX(left[n] + right[n], up[n] + down[n]);
