...
{
    if (VAR(xaxis) > 0 && GOODRAND_01() < 0.5) {
      FPx = VAR(xscale) * (-FPx - VAR(xshift));
      FPy = VAR(yscale) * FPy;
      TC = fmod(TC + VAR(xcolorshift), 1.0);
    }

    if (VAR(yaxis) > 0 && GOODRAND_01() < 0.5) {
      FPx = VAR(xscale) * FPx;
      FPy = VAR(yscale) * (-FPy - VAR(yshift));
      TC = fmod(TC + VAR(ycolorshift), 1.0);
    }

    if (VAR(zaxis) > 0 && GOODRAND_01() < 0.5) {
      FPz = -FPz - VAR(zshift);
      TC = fmod(TC + VAR(zcolorshift), 1.0);
    }
