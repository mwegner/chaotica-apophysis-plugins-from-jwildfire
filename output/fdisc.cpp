...
/*
    Apophysis Plugin: fdisc

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/FDiscFunc.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    */
    ...
    double afactor = M_2PI / ((sqrt(FTx * FTx + FTy * FTy) + SMALL_EPSILON) + VAR(_ashift));
    double r = (atan2(FTy, FTx) * M_1_PI + VAR(_rshift)) * 0.5;
    double xfactor = cos(afactor + VAR(_xshift));
    double yfactor = sin(afactor + VAR(_yshift));
    double pr = VVAR * r;
    double prx = pr * xfactor;
    double pry = pr * yfactor;

    FPx += (VAR(_term1) * prx) + (VAR(_term2) * FTx * prx) + (VAR(_term3) * FTx * pr) + (VAR(_term4) * FTx);
    FPy += (VAR(_term1) * pry) + (VAR(_term2) * FTy * pry) + (VAR(_term3) * FTy * pr) + (VAR(_term4) * FTy);
    // FPx += FTx * VVAR * r * xfactor;
    // FPy += FTy * VVAR * r * yfactor;
