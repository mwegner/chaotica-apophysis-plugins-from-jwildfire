...
/*
    Apophysis Plugin: oscilloscope2

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Oscilloscope2Func.java
...
{
	double separation;
	double frequencyx; // M_PI
	double frequencyy; // M_PI
	double amplitude;
	double perturbation;
	double damping;
	double _tpf;
...
#define APO_VARIABLE_PREFIX "oscilloscope2_"
#include "plugin.h"

APO_PLUGIN("oscilloscope2");
APO_VARIABLES(
	VAR_REAL(separation, 1.00),
	VAR_REAL(frequencyx, M_PI),
	VAR_REAL(frequencyy, M_PI),
...
{
    VAR(_tpf) = 2.0f * M_PI * VAR(frequencyx);
    VAR(_tpf2) = 2.0f * M_PI * VAR(frequencyy);
...
int PluginVarCalc(Variation* vp)
{
    ...
    double t;
    double pt = VAR(perturbation) * sin(VAR(_tpf2) * FTy);
    if (VAR(_noDamping)) {
      t = VAR(amplitude) * (cos(VAR(_tpf) * FTx + pt)) + VAR(separation);
    } else {
      t = VAR(amplitude) * exp(-fabs(FTx) * VAR(damping)) * (cos(VAR(_tpf) * FTx + pt)) + VAR(separation);
    }
