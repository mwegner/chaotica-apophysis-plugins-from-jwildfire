...
int PluginVarCalc(Variation* vp)
{
    ...
    double x = pow((cu * cv), VAR(xpow)) + ((cu * cv) * VAR(xpow)) + (0.25 * at_omega_x);
    double y = pow((su * cv), VAR(ypow)) + ((su * cv) * VAR(ypow)) + (0.25 * at_omega_y);
    double z = pow(sv, VAR(zpow)) + sv * VAR(zpow);
