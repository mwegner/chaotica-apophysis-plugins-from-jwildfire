...
/*
    Apophysis Plugin: ringsubflame

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/RingSubFlameFunc.java
...
APO_PLUGIN("ringsubflame");
APO_VARIABLES(
	VAR_INTEGER(hide_inner, 0),
	VAR_REAL(innerradius, 0.5),
...
void circle2 (Variation* vp, Point p) {
    ...
    //    double r = VAR(innerradius) + VAR(thickness) - this.Gamma * GOODRAND_01();
...
{
    VAR(_radius1) = VAR(innerradius) + VAR(thickness);
    VAR(_radius2) = sqr(VAR(innerradius)) / VAR(_radius1);
...
int PluginVarCalc(Variation* vp)
{
    ...
    Alpha = VAR(innerradius) / r;

    if (r < VAR(_radius1)) {
      if (VAR(hide_inner) == 0) {
        circle2(vp, VAR(_toolPoint)); // Draw/Hide the circle
      ...
      if (GOODRAND_01() > VAR(contrast) * pow(Alpha, VAR(_absPow))) {
        ...
        FPx += VVAR * Alpha * Alpha * q.x;
        FPy += VVAR * Alpha * Alpha * q.y;
      }
    }
    VAR(_oldx) = FPx;
    VAR(_oldy) = FPy;
    ...
    double rad = sqrt(FPx * FPx + FPy * FPy);
    double ang = atan2(FPy, FPx);
    double rdc = cr + (GOODRAND_01() * 0.5 * ca);

    bool esc = rad > cr;
    bool cr0 = VAR(hide_outer) == 1;
