...
/*
    Apophysis Plugin: juliaq

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/JuliaQFunc.java
...
#define APO_VARIABLE_PREFIX "juliaq_"
#include "plugin.h"
...
int PluginVarCalc(Variation* vp)
{
    ...
    double a = atan2(FTy, FTx) * VAR(_inv_power) + GOODRAND_0X(INT_MAX) * VAR(_inv_power_2pi);
    double sina = sin(a);
    double cosa = cos(a);
    double r = VVAR * pow(sqr(FTx) + sqr(FTy), VAR(_half_inv_power));
    FPx += r * cosa;
    FPy += r * sina;
