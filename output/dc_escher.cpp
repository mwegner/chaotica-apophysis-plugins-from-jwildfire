...
/*
    Apophysis Plugin: dc_escher

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/DC_EscherFunc.java
...
typedef struct
{
	int _n=6;
	int _v=4;
	int _pattern_width = 64;
	int _pattern_height = 80;
	double _mas; // pattern_height / (Math.sqrt(2.0-Math.sqrt(3.0))*Math.sqrt(3.0)/2.0)
	bool _vert_move = true;
	int _tex_x,tex_y;
	double _u1=0.0,u2=0.0;
	double _rrk,rr,d,dk,c,s,u,uu;
	double _ca,sa,k_1,k_2,rk5;
	double _rxyk,rk,rk2,rmk,rmk0,xm,ym;
	double _tt,tf,pn,r1k,r1,h,rnk1,rnk2,rn1,count,tmp,fz;
	double _euc0,euc1,euc2,hyp0,hyp1,hyp2;
	double _pi = 3.14159265;
	double _per=10.0;
	double _kseq; // 2.0 * G.mod(time,per)/per - 1.0
	double _r = 0.0; // resolutionY / 2.0 - 10.0

} Variables;

#define APO_VARIABLE_PREFIX "dc_escher_"
#include "plugin.h"

APO_PLUGIN("dc_escher");
APO_VARIABLES(
...
{
   VAR(_pattern_width) = 64;
   VAR(_pattern_height) = 80;
   VAR(_vert_move) = true;
   VAR(_pi) = 3.14159265;
   VAR(_r) = 0.0;
