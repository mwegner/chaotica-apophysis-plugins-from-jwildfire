...
/*
    Apophysis Plugin: dc_kaleidotile

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/DCKaleidotileFunc.java
...
int PluginVarPrepare(Variation* vp)
{
    ...
    VAR(_q1) = -VAR(pull); // pulls apart the 2 sections of the plugin
    VAR(_w) = VAR(rotate); // rotates both halves of the plugin
    VAR(_w1) = -VAR(rotate); // rotates both halves of the plugin
    VAR(_e) = VAR(line_up);
    VAR(_e1) = -VAR(line_up);
    VAR(_r) = VAR(x); // changes VAR(x) co-ordinates
    VAR(_r1) = -VAR(x); // changes VAR(x) co-ordinates
    VAR(_t) = VAR(y); // changes VAR(y) co-ordinates for 1 part of the plugin
    VAR(_t1) = -VAR(y); // changes VAR(y) co-ordinates for 1 part of the plugin
    VAR(_c11) = (1 + VAR(speed1)) / 2;
    VAR(_c12) = VAR(color1) * (1 - VAR(speed1)) / 2;
    VAR(_c21) = (1 + VAR(speed2)) / 2;
    VAR(_c22) = VAR(color2) * (1 - VAR(speed2)) / 2;
