...
{
    VAR(_starblur_alpha) = M_PI / VAR(power);
    VAR(_starblur_length) = sqrt(1.0 + sqr(VAR(range)) - 2.0 * VAR(range) * cos(VAR(_starblur_alpha)));
    VAR(_starblur_alpha) = asin(sin(VAR(_starblur_alpha)) * VAR(range) / VAR(_starblur_length));
...
int PluginVarCalc(Variation* vp)
{
    ...
    double f = GOODRAND_01() * VAR(power) * 2.0;
    double angle = trunc(f);
    f = f - angle;
    double x = f * VAR(_starblur_length);
    double z = sqrt(1 + sqr(x) - 2 * x * cos(VAR(_starblur_alpha)));
    if (((int) angle) % 2 == 0)
      angle = M_2PI / (double) VAR(power) * (((int) angle) / 2) + asin(sin(VAR(_starblur_alpha)) * x / z);
    else
      angle = M_2PI / (double) VAR(power) * (((int) angle) / 2) - asin(sin(VAR(_starblur_alpha)) * x / z);
    z = z * sqrt(GOODRAND_01());
