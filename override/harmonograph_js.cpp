...
void calcParams (Variation* vp) {

    VAR(a1) = (int) (GOODRAND_0X(180)); // amplitude as you like
    VAR(f1) = -5.0 + GOODRAND_0X(5 - (-5) + 1); //keep frequencies around 1
    VAR(p1) = (int) (GOODRAND_0X(180));
    VAR(d1) = 0.0005 + GOODRAND_01() / 1000.0;

    VAR(a2) = (int) (GOODRAND_0X(180));
    VAR(f2) = -5.0 + GOODRAND_0X(5 - (-5) + 1);
    VAR(p2) = (int) (GOODRAND_0X(180));
    VAR(d2) = 0.0005 + GOODRAND_01() / 1000.0;

    VAR(a3) = (int) (GOODRAND_0X(180));
    VAR(f3) = -5.0 + GOODRAND_0X(5 - (-5) + 1);
    VAR(p3) = (int) (GOODRAND_0X(90));
    VAR(d3) = 0.0005 + GOODRAND_01() / 1000.0;

    VAR(a4) = (int) (GOODRAND_0X(180));
    VAR(f4) = -5.0 + GOODRAND_0X(5 - (-5) + 1);
    VAR(p4) = (int) (GOODRAND_0X(90));
    VAR(d4) = 0.0005 + GOODRAND_01() / 1000.0;
}

double mo (Variation* vp, double t, double a1, double f1, double p_1, double d1, double a2, double f2, double p_2, double d2) {

    double p1 = p_1 * M_PI / 180.0;
    double p2 = p_2 * M_PI / 180.0;

    return VAR(a1) * sin(t * VAR(f1) + VAR(p1)) * exp(-1 * VAR(d1) * t) + VAR(a2) * sin(t * VAR(f2) + VAR(p2)) * exp(-1 * VAR(d2) * t);
}
...
int PluginVarCalc(Variation* vp)
{
    ...
    double x, y;
    double t = (double) (GOODRAND_01() * VAR(time));

    x = mo(vp, t, VAR(a1), VAR(f1), VAR(p1), VAR(d1), VAR(a2), VAR(f2), VAR(p2), VAR(d2)) / 180;
    y = mo(vp, t, VAR(a3), VAR(f3), VAR(p3) + 90.0, VAR(d3), VAR(a4), VAR(f4), VAR(p4) + 90.0, VAR(d4)) / 180;
