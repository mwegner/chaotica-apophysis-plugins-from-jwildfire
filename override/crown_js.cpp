...
int PluginVarCalc(Variation* vp)
{
    ...
    double t = (-M_PI + 2.0 * M_PI * GOODRAND_01());

    Complex wt(0.0, 0.0);

    for (int k = 1; k < 15; k++) {
      double denom = pow(VAR(_a), VAR(_b) * k);
      double th = pow(VAR(_a), (double) k) * pow(-1.0, (double) k) * t;
      Complex other(sin(th) / denom, cos(th) / denom);
      wt = complexAdd(wt, other);
    }
    x = wt.real();
    y = wt.real();
    z = pow(complexMag2(wt), 2);
