/*
    Apophysis Plugin: r_circleblur

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/R_CircleblurFunc.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    //truncate to circle
    double rcn = fabs(VAR(n));
    double angle = atan2(FTy, FTx);
    ...
    by = round(by * rad);
    bx = round(bx * rad);
    double rad2 = sqrt(GOODRAND_01()) * 0.5;
    double angle2 = GOODRAND_01() * M_2PI;
    double a1 = sin(bx * 127.1 + by * 311.7 + VAR(seed)) * 43758.5453;
    a1 = a1 - trunc(a1);
    double a2 = sin(bx * 269.5 + by * 183.3 + VAR(seed)) * 43758.5453;
    a2 = a2 - trunc(a2);
    double a3 = sin(by * 12.9898 + bx * 78.233 + VAR(seed)) * 43758.5453;
    a3 = a3 - trunc(a3);
    a3 = a3 * (VAR(max) - VAR(min)) + VAR(min);
    rad2 *= a3;
    by = by + rad2 * sin(angle2) + a1 * VAR(dist);
    bx = bx + rad2 * cos(angle2) + a2 * VAR(dist);
