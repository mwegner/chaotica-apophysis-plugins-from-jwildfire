/*
    Apophysis Plugin: mandala2

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Mandala2Func.java
...
typedef struct
{
	...
	int _minsky;
	int _wobble_pick;
	int _wrap_range_pick;
	int _extra_hskew_pick;
	int _colorid;
	int _seed = 20000;
	...
	int _skew_maps_min;
	int _skew_maps_max;
	int _show_base = -32;  //  Lowest value for both x and y on screen--upper left corner.
	double _angle;
	double _c;
	double _s;
	double _epsilon;
	...
	int _vskew;
	int _hskew2;
	int _max_n_steps = 20000;
	int _step_counts;
	int _biggest_size;
	bool _boolean_minsky = false;
	double _wobble;
	double _wrap_range;
	int _extra_hskew;
...
void setColorFor_n_steps (Variation* vp, int n_steps) {

    float z, h, VAR(_s), b, r, gr;

    Color color = null;  // Convince the compiler that it will be initialized.

    if (n_steps <= VAR(_max_n_steps) + 1 && colors[n_steps] != null) {
//				  			g.setColor( colors[ n_steps ] );
    } else {
      if (n_steps == VAR(_max_n_steps) + 1) {
        color = Color.black;
      } else {
        if (VAR(_colorid) == 0) {
        ...
        } else if (VAR(_colorid) == 1) {
          z = (float) log(n_steps);
          h = z - (float) floor(z);
          b = (float) (1 - .5 * (z / log(VAR(_max_n_steps))));
          if (b > .5) VAR(_s) = (float) ((1.0 - b) * 2 /* + .5 */);
          else VAR(_s) = 1;
          //		        color = Color.getHSBColor( h, VAR(_s), b );
          color = Color.getHSBColor(h, (float) 0.85, 1);
        } else if (VAR(_colorid) == 2) {  // wrapping--assume all paths cycle
          z = (float) (n_steps / 6.333333);
          h = z - (float) floor(z);
          b = (float) (.25 + .75 / (1 + (float) n_steps / VAR(_max_n_steps)));
          if (b > .5) VAR(_s) = (float) ((1.0 - b) + .5);
          else VAR(_s) = 1;
          color = Color.getHSBColor(h, VAR(_s), b);
          color = Color.getHSBColor(h, (float) 0.85, 1);
        } else if (VAR(_colorid) == 3) {
          b = (float) (n_steps % 17) / 16;
          r = (float) (n_steps % 91) / 90;
          gr = (float) (n_steps % 123) / 122;
          color = new Color(r, gr, b);
      ...
      if (n_steps <= VAR(_max_n_steps) + 1) colors[n_steps] = color;
//				  			g.setColor( color );
    }
}

void RotatorMap (Variation* vp, int width, int height, int colorid) {

    VAR(_width) = VAR(_width);
    VAR(_height) = VAR(_height);
    VAR(_colorid) = VAR(_colorid);
    VAR(_step_counts) = new int[VAR(_width)][VAR(_height)];
    VAR(_show_base) = -VAR(_width) / 2;

}

void rotate (Variation* vp, Point p) {

    double x, y;
    x = p.getX();
    y = p.getY();
    x += (int) round(VAR(_c) * y);
    y += (int) round(VAR(_s) * x);
    x += (int) round(VAR(_c) * y);
    p.setX(x);
    p.setY(y);
}

void set_up_skews (Variation* vp, boolean minsky, double wobble, double wrap_range, int extra_hskew) {



    VAR(_minsky) = (VAR(_minsky)) ? 1 : 0;
    VAR(_wobble) = VAR(_wobble);
    VAR(_wrap_range) = VAR(_wrap_range);
    VAR(_extra_hskew) = VAR(_extra_hskew);
    ...
    // them is a little complicated with "VAR(_wobble)," I make tables out of them.
    ...
    // take an off-screen trip around (0,0).  But this depends on the "VAR(_wrap_range)" parameter.
    // If VAR(_wrap_range) zero, then the skew tables are expanded whenever a point goes out of range.
    // If VAR(_wrap_range) is nonzero, it'VAR(_s) a multiplier for the largest of the screen VAR(_width) or
    //    VAR(_height).  A fixed range is set, the same for both x and y, and x and y values are
    ...
    if (VAR(_width) > VAR(_height)) VAR(_biggest_size) = VAR(_width);
    else VAR(_biggest_size) = VAR(_height);

    if (VAR(_wrap_range) == 0.0) VAR(_skew_maps_size) = VAR(_biggest_size) * 2;
    else VAR(_skew_maps_size) = (int) (VAR(_biggest_size) * VAR(_wrap_range));

    // Make VAR(_skew_maps_size) odd so that it can have a center value:
    VAR(_skew_maps_size) |= 1;

    // Try to center the wrap range around zero, but if it'VAR(_s) too tight for that,
    // adjust the center as little as possible to get the screen inside the range
    // --but the range is kept the same for x and y.
    VAR(_skew_maps_base) = VAR(_skew_maps_size) / 2;
    if (VAR(_skew_maps_base) + (VAR(_show_base) + VAR(_biggest_size)) > VAR(_skew_maps_size)) {
      VAR(_skew_maps_base) = -(VAR(_show_base) + VAR(_biggest_size)) + VAR(_skew_maps_size);
      // System.out.println( "VAR(_skew_maps_base) = " + VAR(_skew_maps_base) );
    }

    VAR(_hskew1) = null;  // Having this null triggers recreating them.
    int x_or_y = adjust_skew_maps(vp, 0);
}

int adjust_skew_maps (Variation* vp, int x_or_y) {

    if (VAR(_wrap_range) == 0.0) {
      // Can't count on VAR(_skew_maps_min) or VAR(_skew_maps_max) here.
      while (VAR(_skew_maps_base) + x_or_y < 0 ||
              VAR(_skew_maps_base) + x_or_y >= VAR(_skew_maps_size)) {
        VAR(_skew_maps_size) *= 2;
        VAR(_skew_maps_base) = VAR(_skew_maps_size) / 2;
        VAR(_hskew1) = null;
      }
    }
    if (VAR(_hskew1) == null) {
      // System.out.println( "adjust_skew_maps(vp,  " + x_or_y + " ) VAR(_skew_maps_size) == " + VAR(_skew_maps_size) );
      VAR(_skew_maps_min) = -VAR(_skew_maps_base);
      VAR(_skew_maps_max) = VAR(_skew_maps_min) + VAR(_skew_maps_size) - 1;
      // System.out.println( "VAR(_skew_maps_min) = " + VAR(_skew_maps_min) + ", _max = " + VAR(_skew_maps_max) );


      VAR(_hskew1) = new int[VAR(_skew_maps_size)];
      VAR(_vskew) = new int[VAR(_skew_maps_size)];
      VAR(_hskew2) = new int[VAR(_skew_maps_size)];

      double lowangle = VAR(_num) / ((1 + VAR(_wobble)) * VAR(_denom));
      double hiangle = VAR(_num) / ((1 - VAR(_wobble)) * VAR(_denom));
      double wobble_frequency = 1 / 512.0;

      for (int i = -VAR(_skew_maps_base); i + VAR(_skew_maps_base) < VAR(_skew_maps_size); i++) {
        double x = i * 2 * M_PI * wobble_frequency;
        VAR(_angle) = M_PI * 2 * (lowangle - (cos(x) - 1) / 2 * (hiangle - lowangle));
        if (VAR(_minsky) == 1) { // eliptical  HAKMEM items 149 (Minsky), 151 (Gosper)
          VAR(_epsilon) = sqrt(2 * (1 - cos(VAR(_angle))));  //
          // See below about the .00000001
          VAR(_hskew1)[i + VAR(_skew_maps_base)] = (int) -round(VAR(_epsilon) * i + .00000001) + VAR(_extra_hskew);
          VAR(_vskew)[i + VAR(_skew_maps_base)] = (int) round(VAR(_epsilon) * i + .00000001);
          VAR(_hskew2)[i + VAR(_skew_maps_base)] = 0;
        } else { // Normal, circular, "Givens":
          VAR(_s) = sin(VAR(_angle));
          VAR(_c) = (cos(VAR(_angle)) - 1) / VAR(_s);
          VAR(_hskew1)[i + VAR(_skew_maps_base)] = (int) round(VAR(_c) * i);  // indexed by y
          // In Java the following things are true:
          // VAR(_s) = sin( M_PI * 2 * ( 1 / 12 ) ) = .49999999999999994
          // round(VAR(_s))       = 1.0   (!?)
          // round( VAR(_s) * 2 ) = 1.0
          // round( VAR(_s) * 3 ) = 1.0
          // This causes a mess in the picture when VAR(_denom) == 12,
          // so this is the fix I came up with:
          VAR(_vskew)[i + VAR(_skew_maps_base)] = (int) round(VAR(_s) * i + .00000001);  // indexed by x
          VAR(_hskew2)[i + VAR(_skew_maps_base)] = VAR(_hskew1)[i + VAR(_skew_maps_base)] + VAR(_extra_hskew);
        }
      }
    }
    return (x_or_y - VAR(_skew_maps_size) *
            (int) floor((double) (VAR(_skew_maps_base) + x_or_y) / VAR(_skew_maps_size))
    );
}

int intrandom_range (Variation* vp, int a, int b) {

    return a + (int) (GOODRAND_01() * (b - a));
}
...
void initOnce (Variation* vp, Layer pLayer, XForm pXForm, double pAmount) {
    ...
    RotatorMap(vp, VAR(_width), VAR(_height), VAR(_colorid));
    if (VAR(_minsky) == 1)
      VAR(_boolean_minsky) = true;

    VAR(_wobble) = VAR(_wobble_dat)[VAR(_wobble_pick)];
    if (VAR(_wobble_pick) == 0)
      VAR(_wobble) = 0.0;
    else
      VAR(_wobble) = pow(10.0, -6.0 + .5 * VAR(_wobble_pick));

    VAR(_wrap_range) = VAR(_wrap_dat)[VAR(_wrap_range_pick)];
    VAR(_extra_hskew) = skew_dat[VAR(_extra_hskew_pick)];

    drawMap(vp, VAR(_num), VAR(_denom), VAR(_boolean_minsky), VAR(_wobble), VAR(_wrap_range), VAR(_extra_hskew));
}

void drawMap (Variation* vp, int num, int denom, boolean minsky, double wobble, double wrap_range,
                      int extra_hskew) {
...
//			Graphics g = canvas.image.getGraphics();
    ...
    VAR(_num) = VAR(_num);
    VAR(_denom) = VAR(_denom);

    // VAR(_angle) = M_PI * 2 * VAR(_num) / VAR(_denom) ... on average, but see:
    set_up_skews(vp, VAR(_minsky), VAR(_wobble), VAR(_wrap_range), VAR(_extra_hskew));

    int offs_x = -VAR(_show_base);
    int offs_y = -VAR(_show_base);
    min_show_x = VAR(_show_base);
    max_show_x = min_show_x + VAR(_width) - 1;
    min_show_y = VAR(_show_base);
    max_show_y = min_show_y + VAR(_height) - 1;
    colors = new Color[VAR(_max_n_steps) + 2];

    for (int x = min_show_x; x <= max_show_x; x++) {
      for (int y = min_show_y; y <= max_show_y; y++) {
        if (VAR(_step_counts)[x + offs_x][y + offs_y] == 0) {
          int px = x;
          int py = y;
//						g.setColor( Color.black );  // show some kind of progress...
          for (n_steps = 1; n_steps <= VAR(_max_n_steps) || VAR(_wrap_range) > 0.0; n_steps++) {
            for (int i_denom = 0; i_denom < VAR(_denom); i_denom++) {
              px += VAR(_hskew1)[py + VAR(_skew_maps_base)];
              if (px < VAR(_skew_maps_min) || px > VAR(_skew_maps_max)) px = adjust_skew_maps(vp, px);
              py += VAR(_vskew)[px + VAR(_skew_maps_base)];
              if (py < VAR(_skew_maps_min) || py > VAR(_skew_maps_max)) py = adjust_skew_maps(vp, py);
              px += VAR(_hskew2)[py + VAR(_skew_maps_base)];
              if (px < VAR(_skew_maps_min) || px > VAR(_skew_maps_max)) px = adjust_skew_maps(vp, px);
            }
            if (px == x && py == y) {  // Found a cycle...
              break;
            }
            if (n_steps > VAR(_max_n_steps)) {
//							 	g.fillRect( px + offs_x, py + offs_y, 1, 1 );
              //						Ngon poly=new Ngon(4,1.0/(float)VAR(_biggest_size),0.0,new Point(px + offs_x, py + offs_y),colors[n_steps],0.0);
              //					    primitives.add(poly);
              n_steps = VAR(_max_n_steps);
            }
          }
          // Now retrace that path, knowing the (non-)cycle length:
          px = x;
          py = y;
          setColorFor_n_steps(vp, n_steps);
          for (int n = 1; n <= n_steps; n++) {
            if (px >= min_show_x && px <= max_show_x &&
                    py >= min_show_y && py <= max_show_y) {
              if (VAR(_step_counts)[px + offs_x][py + offs_y] != 0) {
                break;  //  been there done...something...
              }
              VAR(_step_counts)[px + offs_x][py + offs_y] = n_steps;
            }
            for (int i_denom = 0; i_denom < VAR(_denom); i_denom++) {
              px += VAR(_hskew1)[py + VAR(_skew_maps_base)];
              if (px < VAR(_skew_maps_min) || px > VAR(_skew_maps_max)) px = adjust_skew_maps(vp, px);
...
boolean inrange (Variation* vp, int i1, int i, int i2) {
...
int PluginVarPrepare(Variation* vp)
{
   ...
   VAR(_seed) = 20000;
   VAR(_show_base) = -32;
   VAR(_max_n_steps) = 20000;
   VAR(_boolean_minsky) = false;

    //            	randomize=new Random((long)VAR(_seed));
    //		        GOODRAND_01();
...
int PluginVarCalc(Variation* vp)
{
    ...
    x = intrandom_range(vp, 0, VAR(_width));
    y = intrandom_range(vp, 0, VAR(_height));

    Point p = new Point(x, y);
    if(VAR(_step_counts)!=null) {
      col = colors[VAR(_step_counts)[x][y]];

      if (inrange(vp, 1, VAR(_colorid), 3)) {
      ...
      FPx += VVAR * ((double) (p.getX()) / VAR(_width) - 0.5);
      FPy += VVAR * ((double) (p.getY()) / VAR(_width) - 0.5);
