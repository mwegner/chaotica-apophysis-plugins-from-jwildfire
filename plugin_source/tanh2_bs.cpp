...
int PluginVarCalc(Variation* vp)
{
    ...
    double tanhsin = sin(VAR(y1) * FTy);
    double tanhcos = cos(VAR(y2) * FTy);
    double tanhsinh = sinh(VAR(x1) * FTx);
    double tanhcosh = cosh(VAR(x2) * FTx);
    double d = (tanhcos + tanhcosh);
    if (d == 0) {
      return TRUE;
    }
    double tanhden = 1.0 / d;
    FPx += VVAR * tanhden * tanhsinh;
    FPy += VVAR * tanhden * tanhsin;
