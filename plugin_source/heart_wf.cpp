/*
    Apophysis Plugin: heart_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/HeartWFFunc.java
...
int PluginVarCalc(Variation* vp)
{
    ...
    double nx, t;
    if (a < 0) {
      t = -a / M_PI * T_MAX * VAR(scale_r_left) - VAR(shift_t);
      if (t > T_MAX) {
        t = T_MAX;
      }
      nx = -0.001 * (-t * t + 40 * t + 1200) * sin(M_PI * t / 180.0) * r;
    } else {
      t = a / M_PI * T_MAX * VAR(scale_r_right) - VAR(shift_t);
      if (t > T_MAX) {
        t = T_MAX;
      }
      nx = 0.001 * (-t * t + 40 * t + 1200) * sin(M_PI * t / 180.0) * r;
