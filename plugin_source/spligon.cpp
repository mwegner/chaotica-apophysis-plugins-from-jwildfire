...
int PluginVarPrepare(Variation* vp)
{
    GOODRAND_PREPARE();
   VAR(_r) = 1;
    VAR(_th) = VAR(sides) * M_1_2PI;
    VAR(_thi) = 1.0 / VAR(_th);
    VAR(_j) = 3.14159265358979323846 * VAR(i) / (-2.0 * VAR(sides)) ;
...
{
    double dx,dy;
    double t = VAR(_thi) * floor(atan2(FTx, FTy) * VAR(_th) ) + VAR(_j);
    dx = sin(t); dy = cos(t);
    FPx += VVAR * (FTx + dy * VAR(_r));
    FPy += VVAR * (FTy + dx * VAR(_r));
