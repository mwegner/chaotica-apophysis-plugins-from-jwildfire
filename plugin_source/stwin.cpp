...
{
    GOODRAND_PREPARE();
   VAR(_multiplier) = 0.05;
   VAR(_multiplier2) = 0.0001;
   VAR(_multiplier3) = 0.1;
...
int PluginVarCalc(Variation* vp)
{
    ...
    double x = FTx * VVAR * VAR(_multiplier);
    double y = FTy * VVAR * VAR(_multiplier);
    double x2 = x * x + (VAR(offset_x2) * VAR(_multiplier2));
    double y2 = y * y + (VAR(offset_y2) * VAR(_multiplier2));

    double result = (x2 - y2) * sin(M_2PI * VAR(distort) * (x + y + (VAR(offset_xy) * VAR(_multiplier3))));
