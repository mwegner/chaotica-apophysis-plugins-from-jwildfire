...
int PluginVarPrepare(Variation* vp)
{
    ...
    VAR(_c) = VAR(r) * cos(M_PI / 2.0 * VAR(a)) / VAR(divisor);
    VAR(_d) = VAR(r) * sin(M_PI / 2.0 * VAR(a)) / VAR(divisor);
    VAR(_half_c) = VAR(_c) / 2.0;
    VAR(_half_d) = VAR(_d) / 2.0;
    VAR(_inv_range) = 0.5 / VAR(range);
    VAR(_full_range) = 2 * M_PI * VAR(range);
...
int PluginVarCalc(Variation* vp)
{
    ...
    int n = GOODRAND_0X(VAR(range));
    if (a < 0) n++;
    a += 2 * M_PI * n;
    if (cos(a * VAR(_inv_range)) < GOODRAND_01() * 2.0 - 1.0)
      a -= VAR(_full_range);
