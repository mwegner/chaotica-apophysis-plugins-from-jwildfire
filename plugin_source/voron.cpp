/*
    Apophysis Plugin: voron

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/VoronFunc.java
...
#define APO_VARIABLE_PREFIX "voron_"
#include "plugin.h"
...
int PluginVarCalc(Variation* vp)
{
    ...
    Rmin = 20.0;
    M = (int) floor(FTx / VAR(step));
    N = (int) floor(FTy / VAR(step));
    for (i = -1; i < 2; i++) {
      M1 = M + i;
      for (j = -1; j < 2; j++) {
        N1 = N + j;
        K = (int) (1 + floor(VAR(num) * DiscretNoise(vp, 19 * M1 + 257 * N1 + VAR(xseed))));
        for (l = 0; l < K; l++) {
          X = (DiscretNoise(vp, l + 64 * M1 + 15 * N1 + VAR(xseed)) + M1) * VAR(step);
          Y = (DiscretNoise(vp, l + 21 * M1 + 33 * N1 + VAR(yseed)) + N1) * VAR(step);
    ...
    FPx += VVAR * (VAR(k) * (FTx - X0) + X0);
    FPy += VVAR * (VAR(k) * (FTy - Y0) + Y0);
