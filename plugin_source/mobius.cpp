/*
    Apophysis Plugin: mobius

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/MobiusFunc.java
...
#define APO_VARIABLE_PREFIX "mobius_"
#include "plugin.h"
...
{
    // Mobius, by eralex
    double re_u = VAR(re_a) * FTx - VAR(im_a) * FTy + VAR(re_b);
    double im_u = VAR(re_a) * FTy + VAR(im_a) * FTx + VAR(im_b);
    double re_v = VAR(re_c) * FTx - VAR(im_c) * FTy + VAR(re_d);
    double im_v = VAR(re_c) * FTy + VAR(im_c) * FTx + VAR(im_d);
