/*
    Apophysis Plugin: log_db

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/LogDbFunc.java
...
#define APO_VARIABLE_PREFIX "log_db_"
#include "plugin.h"
...
int PluginVarPrepare(Variation* vp)
{
    ...
    VAR(_denom) = 0.5;
    if (VAR(base) > 1e-20)
      VAR(_denom) = VAR(_denom) / log(2.71828182845905 * VAR(base));
    VAR(_denom) *= VVAR;
    VAR(_fixpe) = M_PI;
    if (VAR(fix_period) > 1e-20)
      VAR(_fixpe) *= VAR(fix_period);
...
int PluginVarCalc(Variation* vp)
{
    ...
    int i;
    for (i = 0; i < 7; i++) {
      adp = GOODRAND_0X(10) - 5; // approximated binomial distr
      if (iabs(adp) >= 3)
        adp = 0; // 0 needs more chances
      fix_atan_period += (double) adp;
    }
    fix_atan_period *= VAR(_fixpe);
    FPx += VAR(_denom) * log((FTx * FTx + FTy * FTy));
    FPy += VVAR * (atan2(FTx, FTy) + fix_atan_period);
