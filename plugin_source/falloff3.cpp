/*
    ...
    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/AbstractFalloff3Func.java
...
const int BT_GAUSSIAN = 0;
const int BT_RADIAL = 1;
const int BT_LOG = 2;
const int BS_CIRCLE = 0;
const int BS_SQUARE = 1;
const double SCATTER_ADJUST = 0.04;
...
#define APO_VARIABLE_PREFIX "falloff3_"
#include "plugin.h"

APO_PLUGIN("falloff3");
APO_VARIABLES(
	VAR_INTEGER(blur_type, 0),
	VAR_INTEGER(blur_shape, 0),
	VAR_REAL(blur_strength, 1.0),
	VAR_REAL(min_distance, 0.5),
	VAR_INTEGER(invert_distance, 0),
	VAR_REAL(mul_x, 1.0),
	VAR_REAL(mul_y, 1.0),
	VAR_REAL(mul_z, 0.0),
	VAR_REAL(mul_c, 0.0),
	VAR_REAL(center_x, 0.0),
	VAR_REAL(center_y, 0.0),
	VAR_REAL(center_z, 0.0),
	VAR_REAL(alpha, 0.0),

);


double sgnd (Variation* vp, double x) {

    return x < 0 ? -1 : 1;
}

double log_scale (Variation* vp, double x) {

    return x == 0 ? 0 : log((fabs(x) + 1.0) * M_E) * sgnd(vp, x) / M_E;
}

double log_map (Variation* vp, double x) {

    return x == 0 ? 0 : (M_E + log(x * M_E)) / 4.0 * sgnd(vp, x);
}

double bs_circle (Variation* vp, Double4 v_in, Double3 center) {
...
double bs_square (Variation* vp, Double4 v_in, Double3 center) {

    return MAX(fabs(v_in.x - center.x), MAX(fabs(v_in.y - center.y), (fabs(v_in.z - center.z))));
}
...
int PluginVarPrepare(Variation* vp)
{
    GOODRAND_PREPARE();
    v_in = new Double4();
    mul = new Double4(VAR(mul_x), VAR(mul_y), VAR(mul_z), VAR(mul_c));
    center = new Double3(VAR(center_x), VAR(center_y), VAR(center_z));
    VAR(_r_max) = SCATTER_ADJUST * VAR(blur_strength);
...
int PluginVarCalc(Variation* vp)
{
    ...
    double weight = VVAR;
    double d_0 = VAR(min_distance);

    Double4 random = new Double4(GOODRAND_01() - 0.5, GOODRAND_01() - 0.5, GOODRAND_01() - 0.5, GOODRAND_01() - 0.5);

    double radius;
    switch (VAR(blur_shape)) {
      case BS_CIRCLE:
        radius = bs_circle(vp, v_in, center);
        break;
      case BS_SQUARE:
        radius = bs_square(vp, v_in, center);
        break;
      default:
        throw new IllegalArgumentException("unsupported VAR(blur_shape) <" + VAR(blur_shape) + ">");
    }

    double dist = MAX(((VAR(invert_distance) != 0 ? MAX(1 - radius, 0) : MAX(radius, 0)) - d_0) * VAR(_r_max), 0);

    Double4 v_out;
    switch (VAR(blur_type)) {
      case BT_GAUSSIAN:
      ...
      case BT_LOG:
        ...
        break;
      default:
        throw new IllegalArgumentException("unsupported VAR(blur_type) <" + VAR(blur_type) + ">");
    }

    // write back output vector
    applyVOut(pAffineTP, pVarTP, v_out, weight);
    // write back output color
    TC = fabs(fmod(v_out.c, 1.0));
