/*
    Apophysis Plugin: grid3d_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/Grid3DWFFunc.java
...
typedef struct
{
	...
	double c5;
	double c6;
	int _random_seed = 123;
	bool _doRotate;

GOODRAND_STRUCT;
} Variables;

#define APO_VARIABLE_PREFIX "grid3d_wf_"
#include "plugin.h"

APO_PLUGIN("grid3d_wf");
APO_VARIABLES(
	VAR_REAL(size, 0.1),
	VAR_REAL(size_spread, 0.0),
	VAR_REAL(spacing, 0.75),
	VAR_REAL(alpha, 0.0),
	VAR_REAL(alpha_spread, 0.0),
	VAR_REAL(beta, 0.0),
	VAR_REAL(beta_spread, 0.0),
	VAR_REAL(gamma, 0.0),
	VAR_REAL(gamma_spread, 0.0),
	VAR_REAL(c1, 0.1),
	VAR_REAL(c2, 0.2),
	VAR_REAL(c3, 0.3),
	VAR_REAL(c4, 0.4),
...
double getSizeSpread (Variation* vp, int pX, int pY, int pZ) {

    if (VAR(size_spread) > EPSILON) {
      ...
      if (storedValue == null) {
        double spread = -GOODRAND_01() * VAR(size_spread) + VAR(size_spread);
        map.put(key, spread);
        return spread;
      } else {
...
double getAlphaSpread (Variation* vp, int pX, int pY, int pZ) {

    if (VAR(alpha_spread) > EPSILON) {
      @SuppressWarnings("unchecked")
      Map<String, Double> map = (Map<String, Double>) RessourceManager.getRessource(getAlphaSpreadMapKey());
      String key = makeXYZKey(pX, pY, pZ);
      Double storedValue = map.get(key);
      if (storedValue == null) {
        double spread = -GOODRAND_01() * VAR(alpha_spread) + VAR(alpha_spread);
        map.put(key, spread);
        return spread;
      } else {
        return storedValue.doubleValue();
...
double getBetaSpread (Variation* vp, int pX, int pY, int pZ) {

    if (VAR(beta_spread) > EPSILON) {
      @SuppressWarnings("unchecked")
      Map<String, Double> map = (Map<String, Double>) RessourceManager.getRessource(getBetaSpreadMapKey());
      String key = makeXYZKey(pX, pY, pZ);
      Double storedValue = map.get(key);
      if (storedValue == null) {
        double spread = -GOODRAND_01() * VAR(beta_spread) + VAR(beta_spread);
        map.put(key, spread);
        return spread;
      } else {
        return storedValue.doubleValue();
...
double getGammaSpread (Variation* vp, int pX, int pY, int pZ) {

    if (VAR(gamma_spread) > EPSILON) {
      @SuppressWarnings("unchecked")
      Map<String, Double> map = (Map<String, Double>) RessourceManager.getRessource(getGammaSpreadMapKey());
      String key = makeXYZKey(pX, pY, pZ);
      Double storedValue = map.get(key);
      if (storedValue == null) {
        double spread = -GOODRAND_01() * VAR(gamma_spread) + VAR(gamma_spread);
        map.put(key, spread);
        return spread;
      } else {
        return storedValue.doubleValue();
...
{
    GOODRAND_PREPARE();
   VAR(_random_seed) = 123;
    VAR(_doRotate) = fabs(VAR(alpha)) > EPSILON || fabs(VAR(beta)) > EPSILON || fabs(VAR(gamma)) > EPSILON || fabs(VAR(alpha_spread)) > EPSILON || fabs(VAR(beta_spread)) > EPSILON || fabs(VAR(gamma_spread)) > EPSILON;
    ...
    sizeSpreadRnd.setSeed(VAR(_random_seed));
    ...
    alphaSpreadRnd.setSeed(VAR(_random_seed));
    ...
    betaSpreadRnd.setSeed(VAR(_random_seed));
    ...
    gammaSpreadRnd.setSeed(VAR(_random_seed));
...
int PluginVarCalc(Variation* vp)
{
    ...
    int cxn = (int) (FTx / VAR(size));
    int cyn = (int) (FTy / VAR(size));
    int czn = (int) (FTz / VAR(size));
    double sizeSpread = getSizeSpread(vp, cxn, cyn, czn);
    double sizescl = VAR(spacing) * (VAR(size) + sizeSpread * 0.1);

    double dx = 0.0, dy = 0.0, dz = 0.0;

    switch (GOODRAND_0X(3)) {
      case 0:
        bool left = GOODRAND_01() < 0.5;
        dx = sizescl * (left ? -0.5 : 0.5);
        dy = sizescl * (GOODRAND_01() - 0.5);
        dz = sizescl * (GOODRAND_01() - 0.5);
        TC = left ? VAR(c1) : VAR(c2);
        break;
      case 1:
        bool top = GOODRAND_01() < 0.5;
        dx = sizescl * (GOODRAND_01() - 0.5);
        dy = sizescl * (top ? -0.5 : 0.5);
        dz = sizescl * (GOODRAND_01() - 0.5);
        TC = top ? VAR(c3) : VAR(c4);
        break;
      case 2:
        bool front = GOODRAND_01() < 0.5;
        dx = sizescl * (GOODRAND_01() - 0.5);
        dy = sizescl * (GOODRAND_01() - 0.5);
        dz = sizescl * (front ? -0.5 : 0.5);
        TC = front ? VAR(c5) : VAR(c6);
        break;
    ...
    if (VAR(_doRotate)) {
      double a = VAR(alpha) + getAlphaSpread(vp, cxn, cyn, czn);
      double b = VAR(beta) + getBetaSpread(vp, cxn, cyn, czn);
      double g = VAR(gamma) + getGammaSpread(vp, cxn, cyn, czn);
      double sina = sin(a);
      double cosa = cos(a);
      double sinb = sin(b);
      double cosb = cos(b);
