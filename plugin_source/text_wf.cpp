/*
    Apophysis Plugin: text_wf

    Port of: https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/variation/TextWFFunc.java
...
APO_PLUGIN("text_wf");
APO_VARIABLES(
	VAR_INTEGER(font_size, 320),
	VAR_REAL(antialias, 0.5),
...
int PluginVarCalc(Variation* vp)
{
    ...
    double DFLT_SCALE = 3.0;
    if (points.size() > 1) {
      Point point = points.get(GOODRAND_0X(points.size()));
      double rawX = point.getX();
      double rawY = point.getY();
      if (VAR(antialias) > 0.01) {
        double dr = (exp(VAR(antialias) * sqrt(-log(GOODRAND_01()))) - 1.0) * 0.001;
        double da = GOODRAND_01() * 2.0 * M_PI;
        rawX += dr * cos(da);
        rawY += dr * sin(da);
      }
      FPx += (rawX * VAR(scale_x) + VAR(offset_x)) * DFLT_SCALE;
      FPy += (rawY * VAR(scale_y) + VAR(offset_y)) * DFLT_SCALE;
    } else {
      FPx += GOODRAND_01();
      FPy += GOODRAND_01();
